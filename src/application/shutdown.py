...
class ShutdownOrchestrator:
    ...
    async def _cancel_x10_batch(self, x10, symbols: list[str], errors: list[str]) -> None:
        ...
        try:
            # ═══════════════════════════════════════════════════════════════
            # FIX: Filter to only X10-valid symbols to prevent "Market not found" errors
            # This happens when _collect_relevant_symbols() returns symbols from both
            # Lighter and X10, but X10 only knows about its own markets.
            # ═══════════════════════════════════════════════════════════════
            x10_markets = set()
            ...
            if hasattr(x10, "mass_cancel_orders"):
                ...
                logger.debug(f"⚠️ [X10 Mass Cancel] Failed for batch of {len(valid_symbols)} markets (may be delisted)")
