...
async def reconcile_positions_atomic(
    state_manager, x10_adapter, lighter_adapter, auto_fix: bool = True
    ...
    try:
        ...
        if auto_fix and zombies:
            for symbol in zombies:
                try:
                    logger.warning(f"⚠️ ZOMBIE: {symbol} - closing in DB (no exchange position)...")
                    ...
                    if trade:
                        ...
                        logger.warning(f"⚠️ ZOMBIE {symbol}: Trade not found in state manager")
        ...
        if ghosts:
            for symbol in ghosts:
                logger.warning(
                    f"👻 GHOST: {symbol} - Position exists on exchange but not in DB! Manual review recommended."
                )
        ...
        try:
            ...
            x10_pos = next(
                (
                    ...
                    if p.get("symbol") == symbol and abs(float(p.get("size", 0) or 0)) > 1e-8
                ),
                None,
            )
