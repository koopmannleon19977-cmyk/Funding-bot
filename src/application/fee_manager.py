# src/domain/services/fee_manager.py
...
class FeeManager:
    ...
    def __init__(self):
        ...
        self._x10_schedule: FeeSchedule | None = None
        self._lighter_schedule: FeeSchedule | None = None
        ...
        logger.info(f"💰 FeeManager initialized (Dynamic Fees: {'ENABLED' if self._enabled else 'DISABLED'})")
    ...
    async def _refresh_x10_fees(self):
        ...
        try:
            ...
            self._x10_schedule = FeeSchedule(
                maker_fee=maker_decimal, taker_fee=taker_decimal, timestamp=time.time(), source="api"
            )
            logger.info(
                f"✅ X10 Fees updated: Maker={float(maker_decimal):.6f}, Taker={float(taker_decimal):.6f} (source=api)"
            )
        except Exception as e:
    ...
    async def _refresh_lighter_fees(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Lighter Fees updated: Maker={float(maker_decimal):.6f}, Taker={float(taker_decimal):.6f} (source=api)"
            )
        except Exception as e:
    ...
    def calculate_trade_fees(
        ...
        actual_fee1: float | Decimal | None = None,
        actual_fee2: float | Decimal | None = None,
    ) -> Decimal:
        """
...
# Singleton getter
_fee_manager: FeeManager | None = None
