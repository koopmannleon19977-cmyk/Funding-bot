...
class LighterStreamConfig:
    ...
    stream_url: str
    market_id: int | None = None  # Market ID for symbol-specific streams
    reconnect_delay_initial: float = 2.0
    reconnect_delay_max: float = 120.0
    ping_interval: float = 20.0
    ping_timeout: float = 10.0
    json_ping_interval: float | None = 10.0  # Lighter uses JSON ping/pong
...
class LighterStreamConnection:
    ...
    async def connect(self) -> None:
        ...
        try:
            self._ws = await websockets.connect(
                url, ping_interval=self.config.ping_interval, ping_timeout=self.config.ping_timeout
            )
    ...
    async def run(self) -> None:
        ...
        while self._running:
            ...
            logger.info(
                f"🔄 [Lighter Stream] Reconnecting {self.stream_type.value} in {delay:.1f}s (attempt {self._metrics['reconnect_count']})"
            )
...
class LighterStreamClient:
    ...
    async def subscribe_to_orderbooks(
        ...
        config = LighterStreamConfig(stream_url=self.stream_url, market_id=market_id)

        connection = LighterStreamConnection(
            LighterStreamType.ORDERBOOK, config, message_handler, on_connect, on_disconnect
        )
    ...
    async def subscribe_to_trades(
        ...
        connection = LighterStreamConnection(
            LighterStreamType.TRADES, config, message_handler, on_connect, on_disconnect
        )
    ...
    async def subscribe_to_funding_rates(
        ...
        connection = LighterStreamConnection(
            LighterStreamType.FUNDING, config, message_handler, on_connect, on_disconnect
        )
