...
class WebSocketOrderClient:
    ...
    def __init__(self, config: WsOrderConfig | None = None):
        ...
        self._on_health_change: Callable[[bool], None] | None = None
        ...
        try:
            while self._running and attempt < max_attempts:
                ...
                if self._reconnect_attempts >= max_attempts:
                    logger.error(
                        f"[WS-ORDER] Max reconnect attempts ({max_attempts}) reached - stopping reconnect loop"
                    )
                    break
                ...
                logger.info(
                    f"[WS-ORDER] Reconnecting in {delay:.1f}s (attempt {self._reconnect_attempts}/{max_attempts})"
                )
    ...
    async def _request_timeout(self, req_id: str):
        ...
        if req_id in self._pending_requests:
            pending = self._pending_requests.pop(req_id)
            if not pending.future.done():
                pending.future.set_exception(TimeoutError(f"Request timeout: {req_id}"))
    ...
    async def send_transaction(
        ...
        message = {"type": "jsonapi/sendtx", "data": {"id": req_id, "tx_type": tx_type, "tx_info": tx_info_obj}}
        ...
        self._pending_requests[req_id] = PendingRequest(future=future, timestamp=time.time(), timeout_task=timeout_task)
    ...
    async def send_batch_transactions(self, tx_types: list[int], tx_infos: list[str]) -> list[WsTransaction]:
        """
...
def create_ws_order_client(url: str | None = None) -> WebSocketOrderClient:
