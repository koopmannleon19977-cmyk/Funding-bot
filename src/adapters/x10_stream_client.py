...
class StreamConfig:
    ...
    depth: int | None = None  # For orderbooks
...
class X10StreamConnection:
    ...
    def __init__(
        ...
        self._metrics = {"messages_received": 0, "reconnect_count": 0, "last_error": None}
    ...
    def _build_headers(self) -> dict[str, str]:
        ...
        headers = {"User-Agent": "X10PythonTradingClient/0.4.5"}
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self._ws = await websockets.connect(
                url, extra_headers=headers, ping_interval=ping_interval, ping_timeout=ping_timeout
            )
        ...
        while self._running:
            try:
                ...
                if close_code in (1000, 1006):
                    ...
                    logger.debug(
                        f"[X10 Stream] Connection closed for {self.stream_type.value} (code={close_code}) - will reconnect"
                    )
                elif close_code or close_reason:
                    logger.warning(
                        f"⚠️ [X10 Stream] Connection closed for {self.stream_type.value} (code={close_code}, reason={close_reason})"
                    )
                else:
    ...
    async def run(self) -> None:
        ...
        while self._running:
            try:
                ...
                if close_code in (1000, 1006):
                    logger.debug(
                        f"[X10 Stream] Connection closed (code={close_code}) for {self.stream_type.value}, will reconnect"
                    )
                else:
            ...
            logger.info(
                f"🔄 [X10 Stream] Reconnecting {self.stream_type.value} in {delay:.1f}s (attempt {self._metrics['reconnect_count']})"
            )
...
class X10StreamClient:
    ...
    def __init__(self, stream_url: str, api_key: str | None = None):
    ...
    async def subscribe_to_account_updates(
        ...
        config = StreamConfig(stream_url=self.stream_url, api_key=self.api_key)

        connection = X10StreamConnection(StreamType.ACCOUNT, config, message_handler, on_connect, on_disconnect)
    ...
    async def subscribe_to_orderbooks(
        ...
        connection = X10StreamConnection(StreamType.ORDERBOOKS, config, message_handler, on_connect, on_disconnect)
    ...
    async def subscribe_to_funding_rates(
        ...
        config = StreamConfig(stream_url=self.stream_url, market_name=market_name)

        connection = X10StreamConnection(StreamType.FUNDING, config, message_handler, on_connect, on_disconnect)
    ...
    async def subscribe_to_public_trades(
        ...
        connection = X10StreamConnection(StreamType.PUBLIC_TRADES, config, message_handler, on_connect, on_disconnect)
    ...
    async def subscribe_to_candles(
        ...
        connection = X10StreamConnection(StreamType.CANDLES, config, message_handler, on_connect, on_disconnect)
    ...
    def get_metrics(self) -> dict[str, dict[str, Any]]:
        ...
        return {key: connection.metrics for key, connection in self._connections.items()}

    def is_connected(self, stream_key: str | None = None) -> bool:
