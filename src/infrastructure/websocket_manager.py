...
import config
from src.utils.helpers import mask_sensitive_data, safe_float
...
class WSConfig:
    ...
    url: str
    name: str
    ping_interval: float | None = 15.0  # Reduced from 20 to 15 for faster detection
    ping_timeout: float | None = 10.0  # Increased from 8 to 10 for tolerance
    ...
    json_ping_interval: float | None = None  # Set to 10.0 for Lighter
...
class ManagedWebSocket:
    ...
    def __init__(
        ...
        self._process_task: asyncio.Task | None = None  # Message processing task
        self._message_queue: asyncio.Queue = asyncio.Queue(
            maxsize=50000
        )  # Increased from 10000 to 50000 for high orderbook volume
    ...
    async def start(self):
        ...
        self._connect_task = asyncio.create_task(self._connection_loop(), name=f"ws_{self.config.name}_connect")
    ...
    async def subscribe(self, channel: str, auth: str | None = None):
            ...
            try:
                ...
                if self.is_connected:
                    ...
                    self._receive_task = asyncio.create_task(
                        self._receive_loop(), name=f"ws_{self.config.name}_receive"
                    )
                    self._heartbeat_task = asyncio.create_task(
                        self._heartbeat_loop(), name=f"ws_{self.config.name}_heartbeat"
                    )
                    self._process_task = asyncio.create_task(
                        self._process_loop(), name=f"ws_{self.config.name}_process"
                    )
                    ...
                    if self.config.name.startswith("x10"):
                        ...
                        asyncio.create_task(self._resubscribe_all(), name=f"ws_{self.config.name}_resubscribe")
                    else:
            ...
            if self._running:
                ...
                if error_code == 1006:
                    ...
                    time_since_last_pong = (
                        now - self._metrics.last_pong_time if self._metrics.last_pong_time > 0 else -1
                    )
                    time_since_last_msg = (
                        now - self._metrics.last_message_time if self._metrics.last_message_time > 0 else -1
                    )
                    time_since_last_ping = (
                        now - self._metrics.last_ping_sent_time if self._metrics.last_ping_sent_time > 0 else -1
                    )
                    ...
                    if time_since_last_pong > 20:
                        logger.warning("    ⚠️ Possible cause: Server stopped responding to pings")
                    if self._metrics.pings_sent == 0:
                        logger.warning("    ⚠️ Possible cause: No pings were sent before disconnect")
                ...
                self._reconnect_delay = min(
                    self._reconnect_delay * self.config.reconnect_delay_multiplier, self.config.reconnect_delay_max
                )
    ...
    async def _connect(self):
        ...
        try:
            ...
            self._ws = await asyncio.wait_for(ws_connect(self.config.url, **connect_kwargs), timeout=30.0)
            ...
            if self.config.ping_interval:
                ...
                logger.info(
                    f"✅ [{self.config.name}] Keepalive enabled: ping_interval={self.config.ping_interval}s, ping_timeout={timeout_str}"
                )
        ...
        try:
            ...
            async for message in self._ws:
                ...
                if self.config.name == "x10_account":
                    ...
                    if isinstance(message, (str, bytes)) and (
                        b'"ping"' in message
                        if isinstance(message, bytes)
                        else '"ping"' in message or '"pong"' in message
                    ):
                ...
                if isinstance(message, str):
                    ...
                    is_pong = (
                        '"type": "pong"' in msg_lower or '"type":"pong"' in msg_lower  # Without spaces
                    )
                    if is_pong:
                        self._metrics.last_pong_time = time.time()
                        self._metrics.pongs_received += 1
    ...
    async def _process_single_message(self, message: str):
        ...
        try:
            ...
            msg_type = (
                data.get("type", "").lower() if isinstance(data.get("type"), str) else str(data.get("type", "")).lower()
            )
            ...
            if msg_type == "ping" or msg_event == "ping":
                if self._ws and self.is_connected:
                    try:
                        ...
                        self._set_health(True)
                        logger.info(
                            f"💓 [{self.config.name}] Received SERVER ping → sent pong (ping #{self._metrics.pongs_received})"
                        )
                    except Exception as e:
    ...
    async def _heartbeat_loop(self):
        ...
        while self._running and self.is_connected:
            try:
                ...
                # ═══════════════════════════════════════════════════════════════════
                if json_ping_interval and self._ws and self.is_connected and now - last_json_ping >= json_ping_interval:
                    try:
                ...
                if self._metrics.last_message_time > 0:
                    ...
                    if silence > stale_threshold and self.config.name != "x10_account":
                        logger.warning(f"🔴 [{self.config.name}] Stream stale ({silence:.0f}s), reconnecting...")
                        self._set_health(False)
                        break
                    else:
                    ...
                    # No messages yet
                    if uptime > 60.0:
                        logger.warning(f"⚠️ [{self.config.name}] No messages received after {uptime:.0f}s")
    ...
    async def _send_subscription(self, channel: str, auth: str | None = None):
...
class WebSocketManager:
    ...
    X10_MARKPRICE_WS_URL = "wss://api.starknet.extended.exchange/stream.extended.exchange/v1/prices/mark"  # Public: Mark Prices (All markets)
    ...
    def __init__(self):
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        self._x10_account_ping_interval: float | None = None  # Disable JSON keepalive pings; rely on protocol pings
        self._x10_account_keepalive_task: asyncio.Task | None = None
    ...
    def _on_websocket_reconnect(self, ws_name: str):
        ...
        if self._orderbook_provider:
            self._orderbook_provider.invalidate_all(reason=f"{ws_name} WebSocket reconnect", exchange=exchange)
        else:
    ...
    async def _x10_account_keepalive_loop(self):
        ...
        if not self._x10_account_ping_interval:
            ...
            return
        logger.info(f"💓 [x10_account] Keepalive loop started (interval={self._x10_account_ping_interval:.0f}s)")
            ...
            try:
                ...
                try:
                    ...
                    if success:
                        ...
                        time_since_pong = (
                            time.time() - self._x10_account_last_pong_time
                            if self._x10_account_last_pong_time > 0
                            else -1
                        )
                        ...
                        logger.warning("⚠️ [x10_account] Failed to send keepalive ping")
                ...
                if not is_healthy:
                    logger.warning("⚠️ [x10_account] Health check failed - triggering reconnect...")
                    ...
                    logger.debug(f"💓 [x10_account] Health OK - msgs={total_msgs} ({counts_str or 'none'})")
    ...
    async def _check_x10_account_health(self) -> bool:
        ...
        conn = self._connections.get("x10_account")
        if not conn:
            logger.warning("⚠️ [x10_account] Connection object not found")
            return False
        ...
        if effective_last_msg > 0:
            ...
            if silence_duration > self._x10_account_stale_threshold:
                ...
                logger.debug(f"💓 [x10_account] Health OK - last activity {silence_duration:.0f}s ago")
    ...
    async def _reconnect_x10_account(self):
        ...
        logger.warning("🔄 [x10_account] Forcing reconnection due to stale connection...")
        ...
        try:
            ...
            logger.info("✅ [x10_account] Reconnection initiated")
    ...
    def _handle_x10_account_pong(self, msg: dict):
        ...
        if "pong" in msg:
            ...
            logger.info(
                f"💓 [x10_account] Received pong #{conn._metrics.pongs_received if conn else '?'}: {pong_value}"
            )
            return True
    ...
    async def start(self, ping_interval: float | None = None, ping_timeout: float | None = None):
        ...
        # 6. X10 MARK PRICE Connection (Public, Real-time prices for all markets)
        x10_markprice_config = WSConfig(
            url=self.X10_MARKPRICE_WS_URL,
            name="x10_markprice",
            ping_interval=15.0,
            ping_timeout=None,
            reconnect_delay_initial=2.0,
        ...
        self._connections["x10_markprice"] = ManagedWebSocket(
            x10_markprice_config,
            self._handle_message,
            on_reconnect=self._on_websocket_reconnect,
        ...
        await asyncio.gather(*[conn.start() for conn in self._connections.values()], return_exceptions=True)
        ...
        if self._x10_account_ping_interval:
            self._x10_account_keepalive_task = asyncio.create_task(
                self._x10_account_keepalive_loop(), name="x10_account_keepalive"
            )
        ...
        self._price_init_task = asyncio.create_task(self._initialize_missing_prices(), name="price_initialization")
    ...
    async def stop(self):
        ...
        await asyncio.gather(*[conn.stop() for conn in self._connections.values()], return_exceptions=True)
    ...
    async def subscribe_x10(
        self, channels: list[str], use_trades_stream: bool = False, use_funding_stream: bool = False
    ):
    ...
    async def subscribe_market_data(self, symbols: list[str]):
        ...
        if lighter_conn:
            ...
            if getattr(config, "LIGHTER_WS_ORDERBOOKS_ENABLED", False):
                ...
                if total_subs >= 95:
                    logger.warning(f"⚠️ [lighter] Approaching subscription limit: {total_subs}/100")
                ...
                logger.info("ℹ️ [lighter] Orderbook data: REST polling only (WS deltas disabled)")
            else:
                logger.info("ℹ️ [lighter] Skipping order_book WS subscriptions (REST polling only)")
        ...
        if x10_trades_conn and x10_funding_conn:
            logger.info("[x10] Connected to Firehose streams (Trades & Funding) for all markets.")
        else:
    ...
    def _get_lighter_market_id(self, symbol: str) -> int | None:
    ...
    async def _handle_message(self, source: str, msg: dict):
        ...
        try:
            ...
            elif source == "x10_markprice":
                # Mark Price Firehose - sendet Echtzeit-Preise für alle Märkte
    ...
    async def _handle_lighter_market_stats(self, msg: dict):
        ...
        if not hasattr(self, "_lighter_market_stats_logged"):
            logger.info(
                f"📊 [WS] First market_stats message: {len(entries)} entries, sample keys: {list(entries[0].keys())[:10] if entries and isinstance(entries[0], dict) else 'N/A'}"
            )
        ...
        for entry in entries:
            ...
            market_id = (
                entry.get("market_id") or entry.get("marketId") or entry.get("market_index") or entry.get("marketIndex")
            )
            ...
            if self.lighter_adapter:
                ...
                price = entry.get("mark_price") or entry.get("last_trade_price") or entry.get("index_price")
                ...
                # ═══════════════════════════════════════════════════════════════════════
                # FIX (2025-12-22): KORREKTE INTERPRETATION der Lighter Funding Rate
                # ═══════════════════════════════════════════════════════════════════════
                #
                ...
                #
                # JETZT (KORREKT):
                #   hourly_rate = raw_rate → Rate ist bereits stündlich!
                #
                # Vergleich mit loris.tools:
                # - XAG Extended: -851.9% (short), Lighter: 124.8% (sell)
                # - Das sind die ANNUALISIERTEN Werte (APY)
                #
                # WICHTIG: Lighter hat ZWEI Funding Rate Felder:
                # - funding_rate: Die LETZTE ANGEWENDETE stündliche Rate (was wir wollen!)
                # - current_funding_rate: Echtzeit-Rate (ändert sich jede Minute, NICHT verwenden!)
                # ═══════════════════════════════════════════════════════════════════════

                # Bevorzuge funding_rate (tatsächlich bezahlte Rate)
                # Fallback auf current_funding_rate nur wenn funding_rate fehlt
                funding_rate = (
                    entry.get("funding_rate")  # ERSTE WAHL: Tatsächlich bezahlte stündliche Rate
                    or entry.get("current_funding_rate")  # FALLBACK: Echtzeit-Rate
    ...
    def _merge_lighter_orderbook_fallback(self, symbol: str, bids: list, asks: list):
        ...
        if sorted_bids and sorted_asks:
            ...
            if best_ask <= best_bid:
                logger.warning(
                    f"⚠️ CROSSED BOOK after fallback merge for {symbol}: ask={best_ask} <= bid={best_bid} - invalidating"
                )
    ...
    async def _handle_x10_message(self, msg: dict):
        ...
        # ═══════════════════════════════════════════════════════════════
        msg_type = (msg.get("type") or msg.get("e") or msg.get("event") or msg.get("channel") or "").upper()
        ...
        if isinstance(data, dict):
            ...
            if "balance" in data and data["balance"]:
                logger.info("💰 [x10_account] Found balance in data")
                await self._handle_x10_balance_update(msg)
    ...
    async def _handle_x10_mark_price(self, msg: dict):
        """Process X10 mark price from /v1/prices/mark stream"""
        data = msg.get("data", msg)  # Handle both {"data": {...}} and direct format
        market = data.get("m", "")
        price = data.get("p")
        ...
        if market and price:
            ...
            if self.x10_adapter:
                ...
                self.x10_adapter._price_cache[symbol] = price_float
                self.x10_adapter.price_cache[symbol] = price_float
    ...
    async def _handle_x10_order_update(self, msg: dict):
        ...
        try:
            ...
            orders = (
                data.get("orders") or data.get("order") or (data if "id" in data or "orderId" in data else None) or []
            )
    ...
    async def _handle_x10_trade_notification(self, msg: dict):
        ...
        try:
            ...
            trades = (
                ...
                or (data if "tradeId" in data or "trade_id" in data or ("id" in data and "price" in data) else None)
                or []
            )
    ...
    async def _handle_x10_balance_update(self, msg: dict):
        ...
        try:
            ...
            balance = (
                ...
                or (data if "equity" in data or "available" in data else None)
                or {}
            )
            ...
            if balance:
                ...
                unrealized_pnl = (
                    balance.get("unrealisedPnl")
                    or balance.get("unrealizedPnl")
                    or balance.get("uPnl")
                    or balance.get("pnl")
                    or "0"
                )
                ...
                logger.debug(
                    f"💰 [x10_account] BALANCE: equity=${equity}, available=${available}, uPnL=${unrealized_pnl}"
                )
    ...
    async def _handle_x10_position_update(self, msg: dict):
        ...
        try:
            ...
            positions = (
                ...
                or (data if "market" in data or "symbol" in data else None)
                or []
            )
            ...
            for pos in positions:
                ...
                entry_price = (
                    ...
                    or pos.get("openPrice")  # Fix #14: Also check openPrice for CLOSED positions
                ...
                unrealized_pnl = (
                    pos.get("unrealisedPnl") or pos.get("unrealizedPnl") or pos.get("uPnl") or pos.get("pnl") or "0"
                )
                ...
                if status == "CLOSED" and (not entry_price or float(entry_price) == 0.0):
                    ...
                    if open_price and float(open_price) > 0:
                        ...
                        logger.debug(
                            f"[x10_account] Fix #14: Using openPrice={open_price} for CLOSED position {market}"
                        )
    ...
    async def _initialize_missing_prices(self):
            ...
            if x10_missing:
                ...
                try:
                    if self.x10_adapter and hasattr(self.x10_adapter, "load_market_cache"):
                        ...
                        if len(still_missing) < len(x10_missing):
                            logger.info(
                                f"✅ Market cache refresh initialized {len(x10_missing) - len(still_missing)} prices"
                            )
                ...
                for symbol in x10_missing:
                    ...
                    try:
                        ...
                        if hasattr(self, "orderbook_provider") and self.orderbook_provider:
                            try:
                                orderbook_snapshot = await self.orderbook_provider.get_x10_orderbook(
                        ...
                        if mid_price is None and self.x10_adapter and hasattr(self.x10_adapter, "fetch_mark_price"):
                            try:
                                # fetch_mark_price is async; use sync accessor here to avoid creating an un-awaited coroutine
                                if hasattr(self.x10_adapter, "fetch_mark_price_sync"):
                                    mark_price = self.x10_adapter.fetch_mark_price_sync(symbol)
                                else:
                                    mark_price = 0
                ...
                if total_initialized > 0:
                    logger.info(
                        f"✅ Initialized {total_initialized}/{len(x10_missing)} missing X10 prices (failed: {total_failed}, now using WebSocket-only)"
                    )
                    if total_failed > 0:
                        # Log which markets failed (for debugging)
                        failed_symbols = [s for s in x10_missing if s not in self.x10_adapter._price_cache]
                        logger.warning(
                            f"⚠️ X10 markets that failed price initialization ({total_failed} total): {failed_symbols}"
                        )
                else:
                    logger.warning(
                        f"⚠️ Could not initialize any X10 prices via REST ({total_failed} failed, {len(x10_missing)} total). Prices will come from WebSocket updates."
                    )
        ...
        if self.lighter_adapter and hasattr(self.lighter_adapter, "market_info"):
            ...
            if lighter_missing:
                logger.info(
                    f"📊 Found {len(lighter_missing)} Lighter markets without prices, trying orderbook fallback..."
                )
                ...
                for symbol in lighter_missing:
                    ...
                    try:
                        ...
                        if hasattr(self, "orderbook_provider") and self.orderbook_provider:
                            try:
                                orderbook_snapshot = await self.orderbook_provider.get_lighter_orderbook(
                                    symbol, force_refresh=True
                                )
                                if orderbook_snapshot and orderbook_snapshot.best_bid and orderbook_snapshot.best_ask:
                                    mid_price = float((orderbook_snapshot.best_bid + orderbook_snapshot.best_ask) / 2.0)
                        ...
                        if (
                            mid_price is None
                            and self.lighter_adapter
                            and hasattr(self.lighter_adapter, "fetch_orderbook")
                        ):
                            try:
                ...
                if lighter_initialized > 0:
                    logger.info(
                        f"✅ Initialized {lighter_initialized}/{len(lighter_missing)} missing Lighter prices via REST (failed: {lighter_failed})"
                    )
    ...
    def _lighter_market_id_to_symbol(self, market_id: int) -> str | None:
...
async def init_websocket_manager(
    ...
    # ═══════════════════════════════════════════════════════════════
    try:
        from src.infrastructure.orderbook_provider import init_orderbook_provider
