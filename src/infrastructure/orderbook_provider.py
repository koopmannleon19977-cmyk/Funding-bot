...
class OrderbookSnapshot:
    ...
    sequence: int | None = None  # Sequence number for detecting gaps
...
class OrderbookProvider:
    ...
    def invalidate_all(self, reason: str = "reconnect", exchange: str | None = None):
        """
    ...
    async def fetch_orderbook_rest_fallback(
        ...
        if time.time() < blacklist_until:
            ...
            logger.warning(f"⏸️ {symbol} temporarily blacklisted for {remaining:.1f}s due to repeated crossed books")
            return None
                                ...
                                if best_ask <= best_bid:
                                    ...
                                    if attempt < max_retries - 1:
                                        ...
                                        if count >= 3:
                                            ...
                                            self._crossed_book_blacklist_until[blacklist_key] = (
                                                time.time() + blacklist_duration
                                            )
                                            logger.error(
                    ...
                    if hasattr(self.x10_adapter, "fetch_orderbook"):
                        ...
                        if orderbook:
                            ...
                            if bids and asks:
                                ...
                                best_ask = float(asks[0][0]) if asks else float("inf")
                                if best_ask <= best_bid:
                                    logger.warning(f"⚠️ REST fallback also returned crossed book for {symbol}")
                                    if attempt < max_retries - 1:
                ...
                if hasattr(self.lighter_adapter, "fetch_orderbook"):
                    ...
                    if orderbook:
                        ...
                        if bids and asks:
                            ...
                            if best_ask <= best_bid:
                                logger.warning(
                                    f"⚠️ REST returned crossed book for {symbol}: ask={best_ask} <= bid={best_bid}"
                                )
                                return None
...
# Singleton instance
_default_provider: OrderbookProvider | None = None
