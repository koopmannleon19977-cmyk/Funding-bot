# src/infrastructure/api/rate_limiter.py - PUNKT 3: TOKEN BUCKET RATE LIMITER (FIXED EXPORTS)
# Note: This file has been moved to infrastructure/api/ for better organization
...
class TokenBucketRateLimiter:
    ...
    def __init__(self, config: RateLimiterConfig | None = None, name: str = "default"):
    ...
    async def acquire(self, tokens: float = 1.0) -> float:
        ...
        if not isinstance(tokens, (int, float)):
            logger.warning(
                f"[{self.name}] acquire() called with non-numeric tokens: {tokens} (type={type(tokens)}). Using 1.0"
            )
            tokens = 1.0
        tokens = float(tokens)
    ...
    def penalize_429(self):
        ...
        logger.warning(
            f"[{self.name}] 429 detected! Penalty: {self._current_penalty:.0f}s (consecutive: {self._consecutive_429s})"
        )
...
X10_RATE_LIMITER = TokenBucketRateLimiter(
    config=RateLimiterConfig(
        tokens_per_second=15.0, max_tokens=50.0, min_request_interval=0.06, penalty_429_seconds=10.0
    ),
    name="X10",
)
...
if lighter_tier == "PREMIUM":
    lighter_config = RateLimiterConfig(
        tokens_per_second=50.0, max_tokens=100.0, min_request_interval=0.02, penalty_429_seconds=10.0
    )
else:
    # ═══════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════
    standard_tokens_per_second = float(getattr(config, "LIGHTER_RATE_LIMIT_TOKENS_PER_SECOND", 1.0))
    standard_max_tokens = float(getattr(config, "LIGHTER_RATE_LIMIT_MAX_TOKENS", 8.0))
    standard_min_interval = float(getattr(config, "LIGHTER_RATE_LIMIT_MIN_INTERVAL", 0.2))
    standard_penalty_seconds = float(getattr(config, "LIGHTER_RATE_LIMIT_PENALTY_SECONDS", 60.0))
...
LIGHTER_RATE_LIMITER = TokenBucketRateLimiter(config=lighter_config, name="LIGHTER")
...
def get_all_stats() -> dict[str, dict]:
    ...
    return {"X10": X10_RATE_LIMITER.get_stats(), "LIGHTER": LIGHTER_RATE_LIMITER.get_stats()}
...
def shutdown_all_rate_limiters():
    ...
    for name, limiter in [("X10", X10_RATE_LIMITER), ("LIGHTER", LIGHTER_RATE_LIMITER)]:
        ...
        limiter.shutdown()
        stats[name] = {"waiters_cancelled": waiter_count, "shutdown": limiter._shutdown}
