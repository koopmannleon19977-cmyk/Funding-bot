...
# Note: This file has been moved to infrastructure/persistence/ for better organization
# ═══════════════════════════════════════════════════════════════════════════════
...
class AsyncDatabase:
    ...
    def __init__(self, config: DBConfig | None = None):
        ...
        self._write_queue: asyncio.Queue[WriteOperation | None] = asyncio.Queue(maxsize=self.config.write_queue_size)
    ...
    async def initialize(self):
        ...
        self._write_task = asyncio.create_task(self._write_loop(), name="db_writer")
    ...
    async def _create_connection(self, readonly: bool = False) -> aiosqlite.Connection:
        ...
        conn = await aiosqlite.connect(self.config.db_path, timeout=self.config.busy_timeout_ms / 1000)
    ...
    async def fetch_one(self, sql: str, params: tuple = ()) -> dict[str, Any] | None:
        ...
        try:
            async with self.read_connection() as conn, conn.execute(sql, params) as cursor:
                row = await cursor.fetchone()
    ...
    async def execute(self, sql: str, params: tuple = (), wait: bool = False) -> int | None:
        """
            ...
            try:
                ...
                try:
                    ...
                    if batch:
                        ...
                        try:
                            op = await asyncio.wait_for(self._write_queue.get(), timeout=0.1)
                            if op is None:  # Shutdown signal
                                break
                            batch.append(op)
                        except TimeoutError:
                ...
                now = time.monotonic()
                should_flush = len(batch) >= self.config.write_batch_size or (
...
class TradeRepository:
    ...
    async def add_trade(self, trade: dict[str, Any]) -> int | None:
    ...
    async def get_trade_by_symbol(self, symbol: str) -> dict[str, Any] | None:
    ...
    async def close_trade(self, symbol: str, pnl: float = 0, funding_collected: float = 0):
        ...
        """
        result = await self.db.execute(sql, (int(time.time() * 1000), pnl, funding_collected, symbol), wait=True)
    ...
    async def get_trade_count(self) -> int:
        ...
        result = await self.db.fetch_one("SELECT COUNT(*) as count FROM trades WHERE status IN ('open','pending')")
        return result["count"] if result else 0

    async def get_total_pnl(self) -> float:
        """Get total realized PnL"""
        result = await self.db.fetch_one("SELECT COALESCE(SUM(pnl), 0) as total FROM trades WHERE status = 'closed'")
        return result["total"] if result else 0.0

    async def save_pnl_snapshot(
        self, total_pnl: float, unrealized_pnl: float, realized_pnl: float, funding_pnl: float, trade_count: int
    ):
        """
        ...
        await self.db.execute(
            sql, (int(time.time() * 1000), total_pnl, unrealized_pnl, realized_pnl, funding_pnl, trade_count)
        )
        logger.debug(
            f"📸 PnL Snapshot saved: total=${total_pnl:.4f}, uPnL=${unrealized_pnl:.4f}, rPnL=${realized_pnl:.4f}"
        )
...
class FundingRepository:
    ...
    async def add_funding_record(self, symbol: str, exchange: str, rate: float, timestamp: int, collected: float = 0):
    ...
    async def get_recent_funding(self, symbol: str, hours: int = 24) -> list[dict[str, Any]]:
    ...
    async def get_rate_history(self, symbol: str, hours: int = 48) -> list[dict[str, Any]]:
...
class ExecutionLogRepository:
    ...
    async def log_execution(
        ...
        """
        await self.db.execute(
            sql, (symbol, action, exchange, 1 if success else 0, order_id, error, latency_ms, int(time.time() * 1000))
        )

    async def get_recent_failures(self, symbol: str, minutes: int = 30) -> int:
...
_is_shutdown: bool = False  # FIX (2025-12-22): Prevent DB re-init during shutdown


def _normalize_db_path(db_path: str | None) -> str:
...
async def get_database(db_path: str | None = None) -> AsyncDatabase:
    ...
    global _db_by_path, _is_shutdown

    # FIX (2025-12-22): Prevent re-initialization during shutdown
    if _is_shutdown:
        raise RuntimeError("Database is shutting down - cannot create new instances")
...
async def get_trade_repository(db_path: str | None = None) -> TradeRepository:
...
async def get_funding_repository(db_path: str | None = None) -> FundingRepository:
...
async def get_execution_repository(db_path: str | None = None) -> ExecutionLogRepository:
...
async def close_database(db_path: str | None = None):
    """Close global database instance(s). If db_path is None, closes all."""
    global _db_by_path, _trade_repo_by_path, _funding_repo_by_path, _execution_repo_by_path, _is_shutdown

    # FIX (2025-12-22): Set shutdown flag FIRST to prevent re-init
    _is_shutdown = True
