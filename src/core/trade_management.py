...
from src.utils.pnl_utils import _side_sign, compute_hedge_pnl
...
def calculate_trade_age(trade: dict) -> tuple:
    ...
    if entry_time:
        ...
        age_seconds = (datetime.now(UTC) - entry_time).total_seconds()
    else:
...
def parse_iso_time(entry_time) -> datetime | None:
    ...
    if isinstance(entry_time, (int, float)):
        entry_time = datetime.fromtimestamp(entry_time, tz=UTC)
...
async def calculate_realized_pnl(
    trade: dict, fee_manager, gross_pnl: float = 0.0, lighter_position: dict | None = None
) -> Decimal:
    """
    ...
    entry_fees = fee_manager.calculate_trade_fees(
        ...
        is_maker1=True,
        is_maker2=False,
        actual_fee1=entry_fee_lighter,
        actual_fee2=entry_fee_x10,
    )
    ...
    exit_fees = fee_manager.calculate_trade_fees(
        ...
        is_maker1=False,
        is_maker2=False,
        actual_fee1=exit_fee_lighter,
        actual_fee2=exit_fee_x10,
    )
...
def _parse_trade_timestamp(ts_val: Any) -> float | None:
...
async def _lighter_fill_stats_for_order(
    ...
    symbol: str,
    order_id: str | None,
    fallback_since_ts: float | None = None,
    ...
    while True:
        try:
            ...
            if fills:
                ...
                total_value = sum(
                    safe_float(t.get("price", 0.0), 0.0) * safe_float(t.get("size", 0.0), 0.0) for t in fills
                )
...
def should_farm_quick_exit(symbol: str, trade: dict, current_spread: float, gross_pnl: float) -> tuple:
    ...
    if current_spread <= spread_threshold:
        if gross_pnl >= min_profit:
            return (
                True,
                f"FARM_PROFIT (age={age_seconds:.0f}s, spread={current_spread * 100:.3f}%, pnl=${gross_pnl:.4f})",
            )
...
async def get_cached_positions(lighter, x10, force=False):
    ...
    try:
        ...
        p_x10, p_lit = await asyncio.wait_for(asyncio.gather(t1, t2, return_exceptions=True), timeout=10.0)
...
async def sync_check_and_fix(lighter, x10, parallel_exec=None):
    ...
    try:
        ...
        x10_symbols = {p.get("symbol") for p in (x10_positions or []) if abs(safe_float(p.get("size", 0))) > 1e-8}
        lighter_symbols = {
                    ...
                    if pos:
                        ...
                        if px > 0:
                            ...
                            if ok:
                                ...
                                logger.warning(
                                    f"⚠️ Failed to close orphaned X10 {sym} (close_live_position returned False)"
                                )
                except Exception as e:
                        ...
                        if pos:
                            ...
                            if px > 0:
                                ...
                                if ok:
                                    ...
                                    logger.warning(
                                        f"⚠️ Failed to close orphaned Lighter {sym} (close_live_position returned False)"
                                    )
                    except Exception as e:
                    ...
                    if p:
                        ...
                        try:
                            ...
                            if ok:
                                ...
                                logger.warning(
                                    f"⚠️ Failed to close X10 zombie {sym} (close_live_position returned False)"
                                )
                        except Exception as e:
                        ...
                        if px > 0:
                            ...
                            try:
                                ...
                                if ok:
                                    ...
                                    logger.warning(
                                        f"⚠️ Failed to close Lighter zombie {sym} (close_live_position returned False)"
                                    )
                            except Exception as e:
...
async def reconcile_db_with_exchanges(lighter, x10):
    ...
    for trade in open_trades:
        ...
        if not on_x10 and not on_lighter:
            logger.warning(f"👻 GHOST TRADE: {symbol} in DB but NOT on exchanges - cleaning DB")
            try:
...
async def reconcile_state_with_exchange(lighter, x10, parallel_exec):
    ...
    try:
        ...
        results = await asyncio.gather(
            lighter.fetch_open_positions(), x10.fetch_open_positions(), return_exceptions=True
        )
        ...
        real_x10 = {
            p.get("symbol"): float(p.get("size", 0)) for p in x10_pos if abs(safe_float(p.get("size", 0))) > 1e-8
        }
            ...
            if symbol not in db_symbols:
                ...
                if symbol in RECENTLY_CLOSED_TRADES:
                    if current_time - RECENTLY_CLOSED_TRADES[symbol] < RECENTLY_CLOSED_PROTECTION_SECONDS:
                        logger.debug(
                            f"⏭️ Skipping {symbol}: Recently closed (grace period {RECENTLY_CLOSED_PROTECTION_SECONDS}s)"
                        )
                        continue
                    ...
                    if abs(l_size) > 0:
                        ...
                        if lighter_position:
                            if lighter_position.get("is_ghost"):
                                logger.debug(
                                    f"👻 Skipping orphan close for {symbol}: Ghost Guardian synthetic position"
                                )
                            ...
                            if px > 0:
                                ...
                                if lighter_position.get("is_dust") and not getattr(config, "IS_SHUTTING_DOWN", False):
                                    ...
                                    if ok:
                                        ...
                                        logger.warning(
                                            f"⚠️ Failed to close orphaned Lighter {symbol} (close_live_position returned False)"
                                        )
                        ...
                        if x10_position:
                            ...
                            if px > 0:
                                ...
                                if ok:
                                    ...
                                    logger.warning(
                                        f"⚠️ Failed to close orphaned X10 {symbol} (close_live_position returned False)"
                                    )
                                orphan_throttle_next_ts[symbol] = now_ts + 30.0
                except Exception as e:
    ...
    for t in trades:
        try:
            ...
            if not x10_order_id or not lighter_order_id:
                logger.debug(
                    f"⏭️ Skipping {sym}: Not fully hedged (x10_id={x10_order_id}, lighter_id={lighter_order_id})"
                )
                continue
                        ...
                        if x10_ob and x10_ob.get("bids") and x10_ob.get("asks"):
                            ...
                            if x10_best_bid > 0 and x10_best_ask > 0:
                                ...
                                if x10_depth < min_depth:
                                    ...
                                    logger.debug(
                                        f"⚠️ {sym} X10: Niedrige Liquidität ${x10_depth:.0f} < ${min_depth:.0f}"
                                    )
                        ...
                        if lit_ob and lit_ob.get("bids") and lit_ob.get("asks"):
                            ...
                            if lit_best_bid > 0 and lit_best_ask > 0:
                                ...
                                if lit_depth < min_depth:
                                    orderbook_depth_ok = False
                                    logger.debug(
                                        f"⚠️ {sym} Lighter: Niedrige Liquidität ${lit_depth:.0f} < ${min_depth:.0f}"
                                    )
            ...
            if not reason:
                ...
                funding_blocked = bool(
                    funding_collected < min_funding_before_exit and not force_close and not shutting_down
                )
            ...
            if not reason:
                ...
                if hold_hours >= max_hold_hours:
                    ...
                    force_close = True
                    logger.warning(
                        f"⚠️ [FORCE CLOSE] {sym}: Max hold time reached ({hold_hours:.1f}h >= {max_hold_hours}h)"
                    )
                        ...
                        try:
                            ...
                            if exit_price_x10 > 0 and exit_price_lighter > 0:
                                ...
                                if fallback_net_pnl >= min_divergence_profit:
                                    ...
                                    logger.info(
                                        f"✅ [PRICE DIVERGENCE FALLBACK] {sym}: Est. Net Profit ${fallback_net_pnl:.2f} >= ${min_divergence_profit:.2f}"
                                    )
                ...
                if not force_close and not reason:
                    if total_pnl < min_profit_exit:
                        logger.debug(f"💎 [HODL] {sym}: Net PnL ${total_pnl:.4f} < ${min_profit_exit:.2f}")
                        ...
                        if not reason and current_apy < min_maintenance_apy:
                            ...
                            logger.info(
                                f"📉 [SMART ROTATION] {sym}: APY dropped to {current_apy * 100:.1f}%. Exiting to free capital."
                            )
                        ...
                        if not reason and t.get("is_farm_trade") and getattr(config, "VOLUME_FARM_MODE", False):
                            should_exit, exit_reason = should_farm_quick_exit(
                                symbol=sym, trade=t, current_spread=current_spread_pct, gross_pnl=gross_pnl
                            )
                        ...
                        if not reason and t.get("is_farm_trade") and getattr(config, "VOLUME_FARM_MODE", False):
                            ...
                            if age_seconds > farm_hold_seconds:
                                ...
                                logger.info(f"✅ [FARM COMPLETE] {sym}: Hold time reached AND profitable!")
                            ...
                            if current_net < 0:
                                ...
                                if state_manager:
                                    ...
                                    if existing is not None:
                                        ...
                                        logger.info(
                                            f"📈 [FUNDING FLIP] {sym}: Rate recovered to POSITIVE! Timer cleared."
                                        )
                ...
                if await close_trade(t, lighter, x10):
                    ...
                    try:
                        ...
                        if t.get("x10_exit_order_id") and hasattr(x10, "get_order_fee"):
                            try:
                                t["exit_fee_x10"] = safe_float(
                                    await x10.get_order_fee(str(t["x10_exit_order_id"])), 0.0
                                )
                            except Exception:
                                pass
                        if t.get("lighter_exit_order_id") and hasattr(lighter, "get_order_fee"):
                            try:
                                t["exit_fee_lighter"] = safe_float(
                                    await lighter.get_order_fee(str(t["lighter_exit_order_id"])), 0.0
                                )
                            except Exception:
                                pass
                    ...
                    realized_funding = safe_float(
                        realized.get("funding_total"), safe_float(t.get("funding_collected") or 0.0, 0.0)
                    )
                    ...
                    await publish_event(TradeClosed(symbol=sym, pnl_usd=float(realized_total)))
