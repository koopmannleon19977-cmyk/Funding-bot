...
class OIMetrics:
    ...
    def is_healthy(self, min_oi: float = 50000) -> bool:
        ...
        return (
            self.current_oi >= min_oi
            and self.trend != OITrend.UNKNOWN
            and abs(self.imbalance) < 0.5  # Not too imbalanced
        )
...
class OpenInterestTracker:
    ...
    async def start(self, update_interval: float = 30.0):
        ...
        self._update_task = asyncio.create_task(self._update_loop(), name="oi_tracker")
        ...
        while not self._shutdown_event.is_set():
            ...
            try:
                ...
                if not symbols:
                    ...
                    try:
                        await asyncio.wait_for(self._shutdown_event.wait(), timeout=self._fetch_interval)
                    ...
                    for sym in batch:
                        ...
                        try:
                            ...
                            if self.lighter:
                                try:
                                    ...
                                    if "429" in str(e):
                                        rate_limited = True
                                        logger.warning(
                                            f"⚠️ OI Tracker: 429 rate limit on Lighter for {sym}, pausing 30s"
                                        )
                                        # Interruptible sleep for rate limit
                                        try:
                                            await asyncio.wait_for(self._shutdown_event.wait(), timeout=30.0)
                            ...
                            if oi_total > 0:
                                # Store snapshot
                                now = time.time()
                                snapshot = OISnapshot(timestamp=now, oi_x10=oi_x10, oi_lighter=oi_lighter)
                                ...
                                total_oi += oi_total
                                logger.debug(
                                    f"✅ {sym}: X10=${oi_x10:,.0f}, Lighter=${oi_lighter:,.0f}, Total=${oi_total:,.0f}"
                                )
                            else:
                                failed_count += 1
                ...
                # ═══════════════════════════════════════════════════════════
                logger.info(
                    f"📊 OI Tracker Cycle {cycle_count}: {updated_count} updated, {failed_count} failed, total OI: ${total_oi:,.0f}"
                )
                ...
                if cycle_count % 3 == 0 or cycle_count == 1:
                    if updated_count > 0:
                        ...
                        logger.info("📈 Top 5 Symbols by Open Interest:")
    ...
    def get_metrics(self, symbol: str) -> OIMetrics | None:
    ...
    def get_oi_data_for_prediction(self, symbol: str) -> dict:
        ...
        if not metrics:
            return {"oi": 0.0, "oi_velocity": 0.0, "oi_trend": "UNKNOWN", "oi_zscore": 0.0, "oi_imbalance": 0.0}
...
# SINGLETON INSTANCE
# ═══════════════════════════════════════════════════════════════
_oi_tracker: OpenInterestTracker | None = None
