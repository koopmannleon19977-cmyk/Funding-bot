...
class AdaptiveThresholdManager:
    ...
    def _recalculate_threshold(self):
        ...
        # ═══════════════════════════════════════════════════════════════════════
        # FIX (2025-12-22): Regime thresholds waren viel zu hoch!
        ...
        if market_avg_apy > 0.25:  # HOT market (>25% avg APY)
            ...
            regime = "HOT"
        elif market_avg_apy > 0.10:  # NORMAL market (10-25% avg APY)
            new_threshold = min_apy_config * 1.2  # Slightly higher
            regime = "NORMAL"
        else:  # COLD market (<10% avg APY)
            new_threshold = min_apy_config * 1.5  # A bit higher, but still reasonable
            regime = "COLD"

        # Smooth the threshold to avoid abrupt jumps
        self.current_threshold = (self.current_threshold * 0.7) + (new_threshold * 0.3)
        # Clamp to min_limit (use MIN_APY_FILTER as floor)
