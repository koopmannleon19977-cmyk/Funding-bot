...
class BotEventLoop:
    ...
    async def start(self):
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "H2",
                            "location": "event_loop.py:112",
                            "message": "start() entry",
                            "data": {"_running": self._running, "shutdown_event_set": self._shutdown_event.is_set()},
                        ...
                        json.dumps(
                            {
                                ...
                                "runId": "run1",
                                "hypothesisId": "H2",
                                "location": "event_loop.py:115",
                                "message": "start() early return - already running",
                                "data": {},
                                "timestamp": int(time.time() * 1000),
                            }
                        )
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "H2",
                            "location": "event_loop.py:127",
                            "message": "About to log BotEventLoop starting",
                            "data": {"_running": self._running, "tasks_count": len(self._tasks)},
                            "timestamp": int(time.time() * 1000),
                        }
                    )
                    + "\n"
        ...
        sorted_tasks = sorted(self._tasks.values(), key=lambda t: t.priority.value)
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "H3",
                            "location": "event_loop.py:147",
                            "message": "Before starting tasks",
                            "data": {
                                "tasks_count": len(sorted_tasks),
                                "enabled_tasks": sum(1 for t in sorted_tasks if t.enabled),
                            },
                            "timestamp": int(time.time() * 1000),
                        }
                    )
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "H3",
                            "location": "event_loop.py:156",
                            "message": "After starting tasks",
                            "data": {"tasks_count": len(self._tasks)},
                            "timestamp": int(time.time() * 1000),
                        }
                    )
                    + "\n"
        ...
        if not self._running:
            logger.error("❌ CRITICAL: _running became False after starting tasks! Re-setting to True.")
            self._running = True
            self._shutdown_event.clear()
            self._shutdown_reason = ""

        logger.info(
            f"🔍 Pre-supervision check: _running={self._running}, _shutdown_event={self._shutdown_event.is_set()}, _shutdown_reason='{self._shutdown_reason}', tasks={len(self._tasks)}"
        )
                        ...
                        json.dumps(
                            {
                                ...
                                "runId": "run1",
                                "hypothesisId": "A",
                                "location": "event_loop.py:154",
                                "message": "Before _supervision_loop",
                                "data": {
                        ...
                        json.dumps(
                            {
                                ...
                                "hypothesisId": "A",
                                "location": "event_loop.py:156",
                                "message": "After _supervision_loop",
                                "data": {"_running": self._running, "shutdown_reason": self._shutdown_reason},
                                "timestamp": int(time.time() * 1000),
                            }
                        )
                        + "\n"
                        ...
                        json.dumps(
                            {
                                ...
                                "runId": "run1",
                                "hypothesisId": "H5",
                                "location": "event_loop.py:170",
                                "message": "CancelledError in supervision loop",
                                "data": {"error": str(e)},
                                "timestamp": int(time.time() * 1000),
                            }
                        )
                        ...
                        json.dumps(
                            {
                                ...
                                "runId": "run1",
                                "hypothesisId": "H5",
                                "location": "event_loop.py:178",
                                "message": "Exception in supervision loop",
                        ...
                        json.dumps(
                            {
                                ...
                                "runId": "run1",
                                "hypothesisId": "H6",
                                "location": "event_loop.py:186",
                                "message": "Before _shutdown()",
                                "data": {"_running": self._running, "shutdown_reason": self._shutdown_reason},
                                "timestamp": int(time.time() * 1000),
                            }
                        )
                        ...
                        json.dumps(
                            {
                                ...
                                "runId": "run1",
                                "hypothesisId": "H6",
                                "location": "event_loop.py:192",
                                "message": "After _shutdown()",
                                "data": {},
                                "timestamp": int(time.time() * 1000),
                            }
                        )
    ...
    def request_shutdown(self):
        ...
        logger.info(
            f"🛑 request_shutdown() called - current _running={self._running}, _shutdown_reason='{self._shutdown_reason}'"
        )
        if not self._shutdown_reason:
        ...
        self._shutdown_event.set()
        logger.info(
            f"🛑 request_shutdown() completed - _running={self._running}, _shutdown_event={self._shutdown_event.is_set()}"
        )
    ...
    async def _start_task(self, managed_task: ManagedTask):
        ...
        try:
            ...
            managed_task.task = asyncio.create_task(coro, name=managed_task.name)
            managed_task.task.add_done_callback(lambda t, mt=managed_task: self._task_done_callback(mt, t))
    ...
    def _task_done_callback(self, managed_task: ManagedTask, task: asyncio.Task):
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "H",
                            "location": "event_loop.py:198",
                            "message": "_task_done_callback",
                            "data": {
                                "task_name": managed_task.name,
                                "_running": self._running,
                                "task_done": task.done(),
                            },
                            "timestamp": int(time.time() * 1000),
        ...
        if exc:
            ...
            try:
                        ...
                        json.dumps(
                            {
                                ...
                                "runId": "run1",
                                "hypothesisId": "H",
                                "location": "event_loop.py:209",
                                "message": "Task exception detected",
                                "data": {
                                    "task_name": managed_task.name,
                                    "error": str(exc),
                ...
                pass
            # #endregion
            logger.error(f"❌ Task {managed_task.name} crashed: {exc}", exc_info=exc)
            if not self._shutdown_reason:
                self._shutdown_reason = f"task_crash:{managed_task.name}"

            # Notify via EventBus
            asyncio.create_task(
                publish_event(NotificationEvent(level="CRITICAL", message=f"Task Crash: {managed_task.name}\n{exc}"))
            )

            # Schedule restart if enabled
            if managed_task.restart_on_failure:
                if managed_task.restart_count < managed_task.max_restarts:
                    asyncio.create_task(self._restart_task(managed_task))
                else:
                    logger.critical(f"Task {managed_task.name} exceeded max restarts ({managed_task.max_restarts})")
    ...
    async def _supervision_loop(self):
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "B",
                            "location": "event_loop.py:258",
                            "message": "_supervision_loop entry",
                            "data": {"_running": self._running, "shutdown_event_set": self._shutdown_event.is_set()},
                            "timestamp": int(time.time() * 1000),
                        }
                    )
        ...
        logger.info(
            f"🔄 Supervision loop starting (running={self._running}, shutdown_event={self._shutdown_event.is_set()})"
        )

        # FIX: Ensure _running is True before entering the loop
        if not self._running:
            logger.error("❌ CRITICAL: _running is False at start of supervision loop! This should never happen.")
        ...
        while self._running:
            ...
            try:
                ...
                try:
                                ...
                                json.dumps(
                                    {
                                        ...
                                        "runId": "run1",
                                        "hypothesisId": "C",
                                        "location": "event_loop.py:263",
                                        "message": "Before wait_for shutdown_event",
                                        "data": {
                                            "_running": self._running,
                                            "shutdown_event_set": self._shutdown_event.is_set(),
                    ...
                    logger.debug(
                        f"🔍 [Iteration {loop_iterations}] Before wait(): _running={running_before_wait}, event_set={event_before_wait}, reason='{reason_before_wait}'"
                    )
                    ...
                    if event_before_wait:
                        ...
                        if not running_before_wait or reason_before_wait:
                            logger.info(
                                f"🛑 [Iteration {loop_iterations}] Breaking: _running={running_before_wait}, reason='{reason_before_wait}'"
                            )
                            break
                        else:
                            # Event set but still running - clear it and continue
                            logger.warning(
                                f"⚠️ [Iteration {loop_iterations}] Clearing unexpected shutdown event and continuing"
                            )
                            self._shutdown_event.clear()
                    ...
                    try:
                        logger.debug(
                            f"⏳ [Iteration {loop_iterations}] Waiting for shutdown event (timeout={health_interval}s)..."
                        )
                        await asyncio.wait_for(self._shutdown_event.wait(), timeout=health_interval)
                        logger.debug(f"✅ [Iteration {loop_iterations}] wait_for() returned (event was set or timeout)")
                    except TimeoutError:
                        ...
                        if self._running and not self._shutdown_reason:
                            logger.debug(f"[Iteration {loop_iterations}] wait_for cancelled while running; continuing")
                            await asyncio.sleep(0.1)
                            continue
                        logger.info(f"[Iteration {loop_iterations}] Cancelled and shutting down - breaking")
                        raise
                    ...
                    logger.debug(
                        f"🔍 [Iteration {loop_iterations}] After wait(): _running={running_after_wait}, event_set={event_after_wait}, reason='{reason_after_wait}'"
                    )
                    ...
                    if not running_after_wait:
                        logger.info(
                            f"🛑 [Iteration {loop_iterations}] Breaking supervision loop: _running={running_after_wait} (shutdown requested)"
                        )
                        break
                    ...
                    if reason_after_wait:
                        logger.info(
                            f"🛑 [Iteration {loop_iterations}] Breaking supervision loop: _shutdown_reason='{reason_after_wait}' (shutdown requested)"
                        )
                        break
                    ...
                    if event_after_wait:
                        logger.warning(
                            f"⚠️ [Iteration {loop_iterations}] Shutdown event set but bot still running - clearing event and continuing"
                        )
                ...
                if now - last_health_check >= health_interval:
                    logger.debug(f"🏥 [Iteration {loop_iterations}] Running health check...")
                            ...
                            json.dumps(
                                {
                                    ...
                                    "runId": "run1",
                                    "hypothesisId": "E",
                                    "location": "event_loop.py:277",
                                    "message": "CancelledError in supervision loop",
                                    "data": {"_running": self._running},
                                    "timestamp": int(time.time() * 1000),
                                }
                            )
                            + "\n"
                ...
                if self._running and not self._shutdown_reason:
                    logger.warning(
                        f"⚠️ [Iteration {loop_iterations}] CancelledError but _running=True - this is unexpected!"
                    )
                    logger.warning("   Attempting to continue the loop...")
                    ...
                    try:
                        ...
                        logger.error(
                            f"❌ [Iteration {loop_iterations}] Cannot continue after CancelledError - task is cancelled"
                        )
                        logger.error("   This indicates the supervision loop task was cancelled externally")
                    ...
                    logger.warning(
                        f"⚠️ [Iteration {loop_iterations}] CancelledError in supervision loop - breaking (shutdown requested)"
                    )
                    break
            except Exception as e:
                # #region agent log
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "run1",
                            "hypothesisId": "G",
                            "location": "event_loop.py:281",
                            "message": "_supervision_loop exit",
                            "data": {
                                "_running": self._running,
        ...
        logger.warning(
            f"⚠️ Supervision loop exited! _running={self._running}, _shutdown_reason='{self._shutdown_reason}', iterations={loop_iterations}"
        )
    ...
    async def _close_all_trades_on_shutdown(self):
        ...
        try:
            ...
            for pos in lighter_pos or []:
                ...
                price = await self.lighter_adapter.fetch_mark_price(symbol)
                notional = abs(size) * float(price) if price else 0.0

                side_of_position = "BUY" if size > 0 else "SELL"

                tasks.append(self.lighter_adapter.close_live_position(symbol, side_of_position, notional))
            ...
            for pos in x10_pos or []:
                ...
                price = await self.x10_adapter.fetch_mark_price(symbol)
                notional = abs(size) * float(price) if price else 0.0
                side_of_position = "BUY" if size > 0 else "SELL"

                logger.info(f"🛑 SHUTDOWN: Closing X10 {symbol} ({size})...")
                tasks.append(self.x10_adapter.close_live_position(symbol, side_of_position, notional))
    ...
    async def _stop_components(self):
        ...
        if self.ws_manager and hasattr(self.ws_manager, "stop"):
            try:
                ...
                logger.error(f"WebSocket stop error: {e.__class__.__name__}: {e}", exc_info=True)
...
# SINGLETON & FACTORY
# ═══════════════════════════════════════════════════════════════
_event_loop: BotEventLoop | None = None
