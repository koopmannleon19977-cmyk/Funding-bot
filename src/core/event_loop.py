...
class BotEventLoop:
    ...
    def _task_done_callback(self, managed_task: ManagedTask, task: asyncio.Task):
        ...
        if exc:
            ...
            # Notify via EventBus
            asyncio.create_task(publish_event(NotificationEvent(
                level="CRITICAL",
                message=f"Task Crash: {managed_task.name}\n{exc}"
            )))
        ...
        while self._running:
            ...
            try:
                ...
                try:
                    ...
                    logger.debug(f"🔍 [Iteration {loop_iterations}] Before wait(): _running={running_before_wait}, event_set={event_before_wait}, reason='{reason_before_wait}'")
                    ...
                    try:
                        logger.debug(f"⏳ [Iteration {loop_iterations}] Waiting for shutdown event (timeout={health_interval}s)...")
                    ...
                    logger.debug(f"🔍 [Iteration {loop_iterations}] After wait(): _running={running_after_wait}, event_set={event_after_wait}, reason='{reason_after_wait}'")
