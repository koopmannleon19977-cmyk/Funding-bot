...
def _get_trade_management():
    ...
    from src.core.trade_management import cleanup_zombie_positions, get_cached_positions
    ...
    while not SHUTDOWN_FLAG:
        try:
            ...
            if time_since_update > WATCHDOG_TIMEOUT:
                logger.warning(
                    f"⚠️ WATCHDOG ALARM: Keine Daten seit {time_since_update:.0f}s! (Limit: {WATCHDOG_TIMEOUT}s)"
                )
                ...
                if time_since_update > WATCHDOG_TIMEOUT * 2:
                    ...
                    if telegram and telegram.enabled:
                        await telegram.send_error("🚨 Bot CRITICAL ERROR - Neustart erforderlich")
                    SHUTDOWN_FLAG = True
                    break
            else:
    ...
    while not SHUTDOWN_FLAG:
        try:
            ...
            async with TASKS_LOCK:
                ...
                finished = [sym for sym, task in ACTIVE_TASKS.items() if isinstance(task, asyncio.Task) and task.done()]
                for sym in finished:
                    try:
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                real_x10_symbols = {
                    p.get("symbol") for p in (x10_pos or []) if abs(safe_float(p.get("size", 0))) > 1e-8
                }
                real_lighter_symbols = {
                    p.get("symbol") for p in (lighter_pos or []) if abs(safe_float(p.get("size", 0))) > 1e-8
                }
...
async def health_reporter(event_loop, parallel_exec):
    ...
    while not SHUTDOWN_FLAG:
        try:
            ...
            await publish_event(NotificationEvent(level="INFO", message=msg))
...
async def logic_loop(lighter, x10, price_event, parallel_exec):
    ...
    from src.core.trade_management import get_cached_positions, reconcile_state_with_exchange
            ...
            if opportunities:
                ...
                if trades_to_launch:
                    ...
                    try:
                        if lighter and hasattr(lighter, "get_open_orders"):
                            for opp in trades_to_launch:
                                ...
                                if sym:
                                    try:
                                        open_orders = await asyncio.wait_for(lighter.get_open_orders(sym), timeout=2.0)
                                        if open_orders and len(open_orders) > 0:
                                            symbols_with_open_orders.add(sym)
                                            logger.warning(
                                                f"⚠️ {sym}: Skipping launch - {len(open_orders)} open order(s) already exist"
                                            )
                                    except TimeoutError:
                    ...
                    for opp in trades_to_launch:
                        ...
                        base_default = (
                            getattr(config, "FARM_NOTIONAL_USD", 12)
                            if is_farm
                            else getattr(config, "DESIRED_NOTIONAL_USD", 16)
                        )
