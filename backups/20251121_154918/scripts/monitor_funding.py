...
FAILED_COINS = {}
OPPORTUNITY_LOG_CACHE = {}  # symbol -> last_log_time
...
async def execute_trade(opp: Dict, lighter: LighterAdapter, x10: X10Adapter) -> bool:
    """
    ✅ PRODUCTION-GRADE EXECUTION mit korrektem Error Handling
    
    Flow:
    1. Pre-checks (Blacklist, Balance, Sizing)
    2. Execute LEG 1
    3. Execute LEG 2 (mit Retry)
    4. Falls LEG 2 failed → Rollback LEG 1
    5. NUR bei Erfolg → DB Write
    """
    if SHUTDOWN_FLAG:
        return False
    ...
    if not ok2:
        ...
        # ✅ CRITICAL: Do NOT write to DB!
        return False

    # ===== 7. DB WRITE (Only after BOTH legs successful) =====
    try:
        ...
        # ✅ Return False so bot knows something went wrong
        return False

    # ===== 8. FEE TRACKING (Background Tasks) =====
    if order_id1:
...
async def logic_loop(lighter, x10, price_event: asyncio.Event):
    ...
    while True:
        try:
            ...
            # ✅ FIXED: Early exit with rate-limited warning
            ...
            # 6. Find Opportunities (nur wenn Platz ist!)
            ...
            for opp in opps:
                if current_open_count >= config.MAX_OPEN_TRADES:
                    break  # Stop if we just reached limit
