...
parallel_exec = None  # Global parallel execution manager
...
def cleanup_finished_tasks():
    ...
    # Wir greifen direkt auf das Dict zu, da wir im Main Loop sind
...
LOCK_MANAGER_LOCK = asyncio.Lock()  # Meta-Lock fr das Dict selbst

async def get_execution_lock(symbol: str) -> asyncio.Lock:
    """
    Thread-safe Lock-Getter fr Symbol-Level Execution.
    
    Args:
        symbol: Trading pair (e.g. "BTC-USD")
    
    async def logic_loop(parallel_exec):
        asyncio.Lock fr das spezifische Symbol
    """
...
async def process_fee_update(adapter, symbol, order_id):
    """
     OPTIMIZED: Background-Task fr Fee-Tracking mit Smart Retry
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Fee ist 0%, knnte aber noch pending sein
...
async def save_current_funding_rates(lighter: LighterAdapter, x10: X10Adapter):
    """Speichert Funding-Rates fr Historik"""
    try:
...
async def get_open_trades() -> List[Dict]:
    """Ldt offene Trades aus In-Memory State (falls vorhanden), sonst aus DB"""
...
async def close_trade(trade: Dict, lighter: LighterAdapter, x10: X10Adapter) -> bool:
    """Schliet einen Trade parallel auf beiden Exchanges"""
    symbol = trade['symbol']
...
async def execute_trade_parallel(opp: Dict, lighter: LighterAdapter, x10: X10Adapter) -> bool:
    """
     PARALLEL EXECUTION with Optimistic Rollback + Prediction v2

    Flow:
    1. Acquire symbol lock
    2. **NEW: Run Prediction Engine v2**
    3. Pre-checks (blacklist, balance, sizing)
    4. Fire BOTH legs simultaneously
    5. If both succeed  DB write
    6. If one fails  Rollback successful leg
    7. Release lock
    """
    if SHUTDOWN_FLAG:
        return False
    ...
    async with lock:
        ...
        # =====  PREDICTION FILTER =====
        # Skip trade if predicted rate is dropping
        ...
        # =====  SMART SIZING WITH PREDICTION CONFIDENCE =====
...
async def cleanup_zombie_positions(lighter, x10):
    ...
    Luft alle 5 Minuten im Logic Loop.
    """
    try:
...
async def sync_db_with_exchanges(lighter, x10):
    ...
    3. Fr jeden DB-Trade ohne Exchange-Position  Setze status='CLOSED'
    
    Dies verhindert "Ghost Trades" nach manuellem Schlieen.
    """
    try:
            ...
            for funding_retry in range(3):
                try:
                    ...
                    if "429" in error_str or "rate limit" in error_str:
                        if funding_retry < 2:
                            # Exponential Backoff fr Funding
    ...
    while True:
        try:
            ...
            for opp in opps:
                ...
                lock = get_task_lock()  # Hole den Lock sicher über die Funktion
                async with lock:
                    if symbol in ACTIVE_TASKS:
                        logger.debug(f"{symbol} already has active task, skipping")
                        continue
                ...
                async with lock:  # Nutze den gleichen Lock erneut
                    ACTIVE_TASKS[symbol] = task
...
async def main():
    ...
    # Parallel Execution Manager (after adapters ready)
