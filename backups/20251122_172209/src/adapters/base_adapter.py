...
class BaseAdapter:
    """Basisklasse fr alle Brsen-Adapter"""
    ...
    def fetch_funding_rate(self, symbol: str) -> Optional[float]:
        """Holt Funding-Rate (STNDLICH normalisiert)"""
        if not getattr(config, "LIVE_TRADING", False):
    ...
    async def open_live_position(
        ...
        """Erffnet Position. Returns: (success, order_id)"""
        if not getattr(config, "LIVE_TRADING", False):
    ...
    async def close_live_position(
        ...
        """Schliet Position. Returns: (success, order_id)"""
        if not getattr(config, "LIVE_TRADING", False):
