...
import config
# Resilient imports: some installations expose `lighter.api`, others `lighter.lighter_api`
try:
...
class LighterAdapter(BaseAdapter):
    def __init__(self):
        ...
        self.orderbook_cache: Dict[str, dict] = {}  # symbol -> orderbook
        ...
        self.semaphore = asyncio.Semaphore(3)  # Reduced from 10 to 3
        self.rate_limiter = AdaptiveRateLimiter(
            initial_rate=3.0,  # Reduced from 8.0
            min_rate=1.0,      # Reduced from 2.0
            max_rate=15.0,
            name="Lighter"
        )
    ...
    async def get_order_fee(self, order_id: str) -> float:
        ...
        Falls du spter Fee-Tracking willst:
        ...
        while True:
            ...
            try:
                ...
                async with session.ws_connect(ws_url) as ws:
                    ...
                    # Subscribe to trades + orderbooks (combined)
                    # First 20 markets for orderbooks to avoid rate limits
                    ...
                    async for msg in ws:
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            ...
                            # Handle orderbook updates (from params field)
                            if "params" in data and isinstance(data["params"], dict):
                                ob_update = data["params"]
                                market_id = ob_update.get("market_id")

                                if market_id and "order_book" in ob_update:
                                    # Find symbol
                                    for sym, info in self.market_info.items():
                                        if info.get('i') == market_id:
                                            ob_data = ob_update["order_book"]
                                            bids = []
                                            asks = []
                ...
                if "429" in error_str or "rate limit" in error_str:
                    ...
                    logger.warning(
                        f" Lighter WS Rate Limited! Backoff: {retry_delay}s "
                        f"(max: {max_delay}s)"
                    )
                else:
        ...
        async with self.semaphore:
            ...
            try:
                ...
                if details and details.order_book_details:
                    ...
                    logger.debug(f" Market {market_id} returned empty")
                    return False
    ...
    async def load_market_cache(self, force: bool = False, max_retries: int = 1):
        ...
        try:
            ...
            SLEEP_BETWEEN_BATCHES = 1.5  # Increased from 0.5
            ...
            for batch_num, i in enumerate(range(0, len(all_ids), BATCH_SIZE), start=1):
                ...
                for retry in range(MAX_RETRIES + 1):
                    try:
                        ...
                        if retry == MAX_RETRIES and batch_failed_ids:
                            ...
                            logger.debug(
                                f" Batch {batch_num}: {len(batch_failed_ids)} markets failed after {MAX_RETRIES+1} attempts"
                            )
                        ...
                        if retry < MAX_RETRIES:
                            logger.warning(f" Lighter Batch {batch_num} Error: {e}. Retrying...")
                ...
                if i + BATCH_SIZE < len(all_ids):  # Don't sleep after last batch
                    await asyncio.sleep(SLEEP_BETWEEN_BATCHES * 1.5)  # Extra safety
    ...
    async def fetch_orderbook(self, symbol: str, limit: int = 20) -> dict:
        """Fetch orderbook from WebSocket cache"""
        try:
            # Check cache
    ...
    async def fetch_open_interest(self, symbol: str) -> float:
        ...
        try:
            ...
            # Use OrderApi to get market stats
            order_api = OrderApi(signer.api_client)
            ...
            if response and response.order_book_details:
                ...
                # Try to get OI from available fields
                ...
                # Fallback: use 24h volume as proxy
    ...
    def min_notional_usd(self, symbol: str) -> float:
        ...
        try:
            ...
            logger.warning(f" {symbol} min_notional_usd Error: {e}")
            return HARD_MIN_USD
    ...
    async def get_real_available_balance(self) -> float:
        ...
        try:
            ...
            if "429" in str(e):
                logger.warning(f" Lighter Balance 429. Cache: ${self._balance_cache:.2f}")
            else:
    ...
    def _scale_amounts(self, symbol: str, qty: Decimal, price: Decimal, side: str) -> Tuple[int, int]:
        ...
        if target_notional < min_required_notional:
            ...
            logger.info(
                f" {symbol} Auto-Bump: {float(min_qty_needed):.8f}  {float(qty):.8f} "
        ...
        if actual_notional < min_notional * Decimal('0.99'):  #  FIX: Decimal('0.99') statt 0.99
            logger.warning(
                f" {symbol} POST-CHECK FAILED: ${float(actual_notional):.2f} < ${float(min_notional):.2f}"
            )
            if step_size > 0:
                ...
                try:
                    ...
                    if err:
                        ...
                        if "nonce" in err_str or "429" in err_str or "too many requests" in err_str:
                            logger.warning(f" Lighter Retry ({attempt+1}/{max_retries+1}): {err}")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not still_open:
                    ...
                    logger.warning(f" {symbol} still open after attempt {attempt+1}")
                    if attempt < max_retries - 1:
                        continue
                    else:
