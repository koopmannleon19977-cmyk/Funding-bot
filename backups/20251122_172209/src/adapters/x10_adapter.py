...
class X10Adapter(BaseAdapter):
    def __init__(self):
        ...
        self.price_cache = {}
        self.funding_cache = {}
        self.orderbook_cache = {}  # symbol -> orderbook data
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                    if resp.status == 200:
                            ...
                            try:
                                ...
                                if notional > 0:
                                    ...
                                    if 0 <= fee_rate <= 0.001:
                                        ...
                                        logger.warning(
                                            f" X10: {order_id} unrealistic fee_rate: {fee_rate*100:.4f}%"
                                        )
                                        return config.TAKER_FEE_X10
                        ...
                        logger.warning(f" X10 Fee Fetch 429 fr {order_id}")
                        return config.TAKER_FEE_X10
                        
                    else:
                        logger.warning(f" X10 Fee Fetch {resp.status} fr {order_id}")
                        return config.TAKER_FEE_X10
                        
        except asyncio.TimeoutError:
            logger.warning(f" X10 Fee Fetch Timeout fr {order_id}")
            return config.TAKER_FEE_X10
        ...
        while True:
            ...
            try:
                ...
                async with session.ws_connect(url) as ws:
                    ...
                    # Subscribe to top 20 markets
                    priority_symbols = list(self.market_info.keys())[:20]
    ...
    def fetch_funding_rate(self, symbol: str):
        """
        X10 Extended: STNDLICHE (1h) Funding Rates.
    ...
    async def fetch_orderbook(self, symbol: str, limit: int = 20) -> dict:
        ...
        try:
            # X10 braucht Underscore statt Dash: BTC-USD  BTC_USD
            ...
            logger.debug(f"X10 orderbook fetch failed: {e}")
        return {"bids": [], "asks": []}
