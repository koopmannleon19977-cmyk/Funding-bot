...
POSITION_CACHE_TTL = 3.0  # Fast cache for realtime decisions
...
async def execute_trade_parallel(opp: Dict, lighter, x10, parallel_exec) -> bool:
    ...
    async with lock:
        ...
        success, id1, id2 = await parallel_exec.execute_trade_parallel(
            symbol, 
        ...
        if success:
            ...
            trade.update({
                ...
                'entry_price_x10': x10.fetch_mark_price(symbol),
                'entry_price_lighter': lighter.fetch_mark_price(symbol),
                'account_label': current_label,
                'initial_funding_rate_hourly': net_rate
            })
            await add_trade_to_state(trade)
            asyncio.create_task(process_fee_update(ex1, symbol, id1))
            asyncio.create_task(process_fee_update(ex2, symbol, id2))
            return True
        else:
...
async def logic_loop(lighter, x10, price_event, parallel_exec):
    ...
    while True:
        try:
            ...
            for s in finished: del ACTIVE_TASKS[s]
