...
from aiohttp import ClientSession  # Für globale Cleanup
...
async def main():
    ...
    try:
        ...
        for sym in test_syms:
            # FIX: Prüfe auch wenn nur eine Exchange verfügbar ist
            in_x10 = sym in x10.market_info
            in_lighter = sym in lighter.market_info
            
            if in_x10 or in_lighter:
                px = x10.fetch_mark_price(sym) if in_x10 else None
                pl = lighter.fetch_mark_price(sym) if in_lighter else None
                
                # OK wenn mindestens eine Exchange Daten hat
                if (px and px > 0) or (pl and pl > 0):
                    logger.info(f"✅ Data OK: {sym} X10=${px or 0:.2f}, Lighter=${pl or 0:.2f}")
                    loaded_ok = True
                    break

        if len(x10.market_info) == 0 and len(lighter.market_info) == 0:
            raise ValueError("No valid market data after load (Both Exchanges Empty)")
        
        if not loaded_ok:
            logger.warning("⚠️ No common symbols checked/found, but at least one exchange has data. Proceeding...")
    ...
    await parallel_exec.start()  # KRITISCH: Startet Background Rollback Processor
    logger.info("✅ ParallelExecutionManager gestartet")
    ...
    try:
        ...
        # Stop ParallelExecutionManager
        try:
            await parallel_exec.stop()
        except Exception as e:
            logger.debug(f"ParallelExec stop error: {e}")
        ...
        # Erhöhe Sleep nach close
        # Globale Session Cleanup (Fallback)
        active_sessions = [s for s in gc.get_objects() if isinstance(s, ClientSession) and not s.closed]
