...
class X10Adapter(BaseAdapter):
    ...
    async def _ws_trades(self, url, retry_delay, max_delay):
        ...
        while True:
            ...
            try:
                ...
                logger.error(f"❌ X10 Trades WS: {e}. Reconnect in {retry_delay}s")
    ...
    async def _ws_orderbook(self, url, retry_delay, max_delay):
        ...
        while True:
            ...
            try:
                ...
                logger.error(f"❌ X10 Orderbook WS: {e}. Reconnect in {retry_delay}s")
                await asyncio.sleep(retry_delay)
            finally:
                if session and not session.closed:
                    await session.close()
    ...
    async def _get_ws_urls(self) -> dict:
        ...
        return {
            ...
            "trades": "wss://api.starknet.extended.exchange/stream.extended.exchange/v1/trades",
    ...
    async def fetch_orderbook(self, symbol: str, limit: int = 20) -> dict:
        ...
        try:
            base_url = getattr(config, 'X10_API_BASE_URL', 'https://api.starknet.extended.exchange')
            url = f"{base_url}/info/orderbook"
            params = {"market": symbol, "depth": limit}
