...
class AdaptiveThresholdManager:
    ...
    def __init__(self, window_size: int = 100):
        ...
        self.taker_fee = getattr(config, 'TAKER_FEE', 0.0005)
        self.maker_fee = getattr(config, 'MAKER_FEE', 0.0)
    ...
    def get_threshold(self, symbol: Optional[str] = None, is_maker: bool = False) -> float:
        ...
        if discount > 0:
            ...
            logger.debug(
                f"Applying rebate discount for {symbol or 'unknown'}: {discount*100:.2f}% -> threshold {base*100:.2f}%"
            )
