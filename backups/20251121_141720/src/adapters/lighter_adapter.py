...
class LighterAdapter(BaseAdapter):
    ...
    async def get_order_fee(self, order_id: str) -> float:
        """Lighter Standard Account = 0% Fees"""
        return 0.0000
    ...
    def _scale_amounts(self, symbol: str, qty: Decimal, price: Decimal, side: str) -> Tuple[int, int]:
        """
        🔧 UNIVERSAL FIX: Skaliert Quantity & Price mit ROUND UP für Quantity
        """
        ...
        min_notional = float(data.get('min_notional', 10.0))
        
        # ===== 1. QUANTITY ROUNDING (ROUND UP!) =====
        if step_size > 0:
            # Round UP zur nächsten step_size
            qty = (qty / step_size).quantize(Decimal('1'), rounding=ROUND_UP) * step_size
        ...
        # ===== 3. SCALING (CRITICAL: Use ROUND_UP for base!) =====
        # 🔧 KEY FIX: int() macht FLOOR → wir brauchen ROUND_UP!
        scaled_base = int((qty * (Decimal(10) ** sd)).quantize(Decimal('1'), rounding=ROUND_UP))
        scaled_price = int(price * (Decimal(10) ** pd))
        
        # ===== 4. MIN-NOTIONAL VALIDATION & AUTO-BUMP =====
        actual_qty = Decimal(scaled_base) / (Decimal(10) ** sd)
        actual_price = Decimal(scaled_price) / (Decimal(10) ** pd)
        actual_notional = float(actual_qty * actual_price)
        
        if actual_notional < min_notional * 0.99:  # 1% Toleranz
            logger.warning(
                f"⚠️ {symbol}: Notional ${actual_notional:.2f} < Min ${min_notional:.2f}"
            )
            
            # Berechne benötigte Quantity
            min_qty_needed = Decimal(str(min_notional)) / actual_price
            
            # Round UP mit Step Size
            if step_size > 0:
                min_qty_needed = (min_qty_needed / step_size).quantize(Decimal('1'), rounding=ROUND_UP) * step_size
            
            # Re-scale mit ROUND_UP
            scaled_base = int((min_qty_needed * (Decimal(10) ** sd)).quantize(Decimal('1'), rounding=ROUND_UP))
            
            # Verify
            new_qty = Decimal(scaled_base) / (Decimal(10) ** sd)
            new_notional = float(new_qty * actual_price)
            logger.info(
                f"⬆️ {symbol}: Bumped qty to meet min_notional "
                f"(${actual_notional:.2f} → ${new_notional:.2f})"
            )
    ...
    async def open_live_position(
        ...
        try:
            ...
            limit_price = Decimal(str(price)) * (
                Decimal(1) + slippage/100 if side == 'BUY' else Decimal(1) - slippage/100
            )
