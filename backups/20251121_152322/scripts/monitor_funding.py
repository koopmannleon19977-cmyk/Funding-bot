...
async def process_fee_update(adapter, symbol, order_id):
    """Background-Task für Fee-Tracking"""
...
async def execute_trade(opp: Dict, lighter: LighterAdapter, x10: X10Adapter) -> bool:
    """Führt Trade aus mit Error Handling & Blacklist"""
    if SHUTDOWN_FLAG:
        return False
    ...
    if absolute_min_required > config.MAX_TRADE_SIZE_USD:
        logger.debug(f"⭐️ Skip {symbol}: Min ${absolute_min_required:.2f} > Max ${config.MAX_TRADE_SIZE_USD:.2f}")
        return False
    ...
    if not ok2:
        ...
        logger.warning(f"⚠️ {symbol} Execution Error. Setze Cooldown für 30s...")
    ...
    try:
        ...
        logger.critical(f"❌ FATAL DB ERROR: Konnte Trade nicht speichern!")
        FAILED_COINS[symbol] = time.time() + 600
        return True
...
async def cleanup_zombie_positions(lighter, x10):
    """
    🔧 INTELLIGENT ZOMBIE CLEANUP

    Ein Zombie ist:
    - Position auf Exchange OHNE entsprechenden DB-Eintrag

    Kein Zombie ist:
    - Position auf Exchange MIT DB-Eintrag (status='OPEN')
    - Beide Legs eines Hedge-Trades (normal)

    Flow:
    1. Hole alle Exchange-Positionen (X10 + Lighter)
    2. Hole alle DB-Trades (status='OPEN')
    3. Finde Symbols die auf Exchange ABER NICHT in DB sind
    4. Close nur diese Symbols auf ALLEN Exchanges
    """
    try:
        logger.info("🧟 Starting Zombie Cleanup...")
        ...
        # Debug Info
        logger.debug(f"📊 X10 Positions: {len(x10_positions or [])}")
        logger.debug(f"📊 Lighter Positions: {len(lighter_positions or [])}")

        # Sammle alle Symbols mit offenen Positionen
        exchange_symbols = set()

        x10_pos_map = {}  # symbol → position_data
        lighter_pos_map = {}
        ...
        if not exchange_symbols:
            logger.debug("✅ No exchange positions found - cleanup done")
            return

        logger.debug(f"🔍 Found positions for: {exchange_symbols}")

        # ===== 2. HOLE DB TRADES =====
        db_trades = await get_open_trades_from_db()
        db_symbols = {t['symbol'] for t in db_trades}

        logger.debug(f"📚 DB has OPEN trades for: {db_symbols}")

        # ===== 3. FINDE ECHTE ZOMBIES =====
        # Zombie = auf Exchange ABER NICHT in DB
        zombie_symbols = exchange_symbols - db_symbols

        if not zombie_symbols:
            logger.info("✅ No zombies found - all positions tracked in DB")
            return

        # ===== 4. WARNUNG + CLOSE ZOMBIES =====
        logger.warning(f"🧟 ZOMBIES DETECTED: {zombie_symbols}")

        for symbol in zombie_symbols:
            logger.info(f"🧹 Cleaning zombie: {symbol}")
            ...
            if symbol in x10_pos_map:
                ...
                if notional > 0:
                    ...
                    logger.info(f"   Closing X10 {symbol}: {size:.6f} ({side})")

                    try:
                        success, oid = await x10.close_live_position(
                            symbol,
                            "BUY" if size > 0 else "SELL",  # original_side (opposite of close)
                            notional
                        )
                        if success:
            ...
            if symbol in lighter_pos_map:
                ...
                if notional > 0:
                    side = "SELL" if size > 0 else "BUY"
                    logger.info(f"   Closing Lighter {symbol}: {size:.6f} ({side})")

                    try:
                        success, oid = await lighter.close_live_position(
                            symbol,
                            "BUY" if size > 0 else "SELL",  # original_side
                            notional
                        )
                        if success:
            ...
            # Warte kurz zwischen Closes
            await asyncio.sleep(0.5)

        logger.info(f"✅ Zombie Cleanup Complete: {len(zombie_symbols)} cleaned")

        # ===== 5. OPTIONAL: CHECK FOR DB-ONLY ZOMBIES =====
        # (Trades in DB aber nicht auf Exchange - sollte nicht passieren)
        db_only_zombies = db_symbols - exchange_symbols

        if db_only_zombies:
            logger.warning(f"🗄️ DB-ONLY ZOMBIES (no exchange position): {db_only_zombies}")
            # ✅ FIX: Close sie in der DB
            async with aiosqlite.connect(config.DB_FILE) as conn:
                for sym in db_only_zombies:
                    logger.info(f"   Closing DB zombie: {sym}")
                    await conn.execute(
            ...
            logger.info(f"   ✅ Marked {len(db_only_zombies)} DB-only zombies as CLOSED")
            # Archive zu History
            for sym in db_only_zombies:
                trade_data = next((t for t in db_trades if t['symbol'] == sym), None)
                if trade_data:
                    await archive_trade_to_history(
                        trade_data,
                        close_reason="DB Zombie (no exchange position)",
                        pnl_data={
                            'total_net_pnl': 0.0,
                            'funding_pnl': 0.0,
                            'spread_pnl': 0.0,
...
async def logic_loop(lighter, x10, price_event: asyncio.Event):
    ...
    while True:
        try:
            ...
            # 2. Zombie Cleanup - ✅ RE-ENABLED!
            if now - last_zombie_check > 300:  # Every 5 minutes
...
async def main():
    ...
    # ✅ NEU: Sync DB mit Exchanges beim Startup
