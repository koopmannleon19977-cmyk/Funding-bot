...
async def execute_trade(opp: Dict, lighter: LighterAdapter, x10: X10Adapter) -> bool:
    """
    PRODUCTION-GRADE EXECUTION with execution lock

    Flow:
    1. Acquire symbol lock (prevents parallel execution)
    2. Pre-checks (blacklist, balance, sizing)
    3. Execute LEG 1
    4. Execute LEG 2 (with retry)
    5. If LEG 2 failed -> rollback LEG 1
    6. Only on success -> DB write
    7. Release lock
    """
    if SHUTDOWN_FLAG:
        return False
    ...
    async with lock:  # CRITICAL SECTION START
        ...
        if absolute_min_required > config.MAX_TRADE_SIZE_USD:
            logger.debug(
                f"Skip {symbol}: Min ${absolute_min_required:.2f} > "
        ...
        if bal1 < min_required_per_exchange:
            logger.debug(
                f"{symbol}: {ex1.name} balance insufficient "
        ...
        if bal2 < min_required_per_exchange:
            logger.debug(
                f"{symbol}: {ex2.name} balance insufficient "
        ...
        if not success1:
            logger.warning(f"{symbol} LEG 1 ({ex1.name}) failed")
            FAILED_COINS[symbol] = time.time()
            return False

        logger.info(f"{symbol} LEG 1 ({ex1.name}) success: {order_id1 or 'OK'}")

        # ===== EXECUTION LEG 2 (with Retry) =====
        ok2 = False
        order_id2 = None
        ...
        for retry_attempt in range(2):
            try:
                ...
                if s2:
                    ...
                    logger.warning(f"{symbol} LEG 2 ({ex2.name}) attempt {retry_attempt+1}/2 failed")
        ...
        # ===== DB WRITE (Only after BOTH legs successful) =====
        try:
            await write_detailed_trade_to_db(trade)
            logger.info(f"{symbol} saved to DB as OPEN")

            # Invalidate position cache after execution
...
async def get_cached_positions(lighter, x10, force_refresh: bool = False):
    """Return cached positions for X10 and Lighter. Fetches from exchanges when cache expired or forced.
