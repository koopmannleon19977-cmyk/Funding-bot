...
TASK_LOCK = None
FEE_CACHE: Dict[str, Dict[str, float]] = {'X10': {}, 'Lighter': {}}
...
async def execute_trade_task(opp: Dict, lighter, x10, parallel_exec):
    symbol = opp['symbol']
    try:
        return await execute_trade_parallel(opp, lighter, x10, parallel_exec)
    finally:
        lock = get_task_lock()
        async with lock:
    ...
    try:
        async with aiosqlite.connect(config.DB_FILE) as conn:
            ...
            if row:
                ...
                new_avg = fee_rate
                new_count = 1
            FEE_CACHE.setdefault(exchange_name, {})[symbol] = new_avg
...
async def load_fee_cache():
    """Load fee stats from DB into memory on startup"""
    try:
        async with aiosqlite.connect(config.DB_FILE) as conn:
        ...
        logger.info(f" Fee Cache loaded ({len(FEE_CACHE.get('X10', {})) + len(FEE_CACHE.get('Lighter', {}))} entries)")
    except Exception as e:
        logger.error(f"Failed to load fee cache: {e}")

def get_dynamic_fee_rate(exchange: str, symbol: str) -> float:
    """Get EMA fee rate from cache or fallback to config"""
...
async def execute_trade_parallel(opp: Dict, lighter: LighterAdapter, x10: X10Adapter, parallel_exec) -> bool:
    if SHUTDOWN_FLAG:
        return False
    ...
    async with lock:
        ...
        # Ensure we have the latest price in monitor
        current_price = x10.fetch_mark_price(symbol)
        if current_price:
            vol_monitor.update_price(symbol, current_price)
            
        safety = vol_monitor.check_trade_safety(symbol)
        if not safety['allow_entry']:
            logger.warning(f" {symbol} VOLATILITY BLOCK: {safety['reason']}")
            return False
            
        # Adjust size based on regime
        size_multiplier = safety.get('size_multiplier', 1.0)
        ...
        if opp.get('is_farm_trade'):
            ...
            target_usd = calculate_smart_size(confidence, total_balance) * size_multiplier
...
async def manage_open_trades(lighter, x10):
    ...
    try:
        x10_positions, lighter_positions = await get_cached_positions(
            lighter, x10, force_refresh=False
        )
    ...
    for t in active_trades:
        ...
        fee_rate_leg1 = get_dynamic_fee_rate(t['leg1_exchange'], sym)
        fee_rate_leg2 = get_dynamic_fee_rate('Lighter' if t['leg1_exchange'] == 'X10' else 'X10', sym)
        # Estimate entry + exit fees (2 round trips approx, or entry+exit)
        estimated_fees = t['notional_usd'] * (fee_rate_leg1 + fee_rate_leg2) * 2.0
        
        total_pnl = funding_pnl + spread_pnl - estimated_fees
        
        reason = None
        
        # Volatility Emergency Exit
        vol_monitor = get_volatility_monitor()
        try:
            if vol_monitor.should_close_due_to_volatility(sym):
                reason = f"EXTREME VOLATILITY ({vol_monitor.current_regimes.get(sym, 'UNKNOWN')})"
        except Exception:
            # If volatility monitor fails, don't block normal checks; log quietly
            logger.debug(f" Vol monitor check failed for {sym}: {traceback.format_exc()}")
...
# ============================================================
async def volume_farm_mode(lighter, x10, open_syms, parallel_exec):
...
async def find_opportunities(lighter, x10, open_syms):
    ...
    for s in common:
        ...
        # X10 is Post-Only (Maker), Lighter is Taker
        is_maker_strat = True 
        req_apy = threshold_manager.get_threshold(s, is_maker=is_maker_strat)
        
        if apy < req_apy:
            reasons["low_apy"] += 1 # Logged as low APY relative to dynamic threshold
            continue
...
async def logic_loop(lighter, x10, price_event: asyncio.Event, parallel_exec):
    ...
    while True:
        try:
            try:
                await asyncio.wait_for(price_event.wait(), timeout=0.5)
            ...
            last_manage_check = locals().get('last_manage_check', 0)
            if now - last_manage_check > MANAGE_TRADES_COOLDOWN:
                await manage_open_trades(lighter, x10)
                last_manage_check = now
            ...
            if now - last_farm_check > 10:
                if not trade_executed and current_open_count < config.MAX_OPEN_TRADES:
                    await volume_farm_mode(lighter, x10, open_symbols, parallel_exec)
                last_farm_check = now
