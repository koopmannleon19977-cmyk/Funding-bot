            ...
            async with conn.execute("SELECT * FROM trades WHERE status = 'OPEN'") as cursor:
                ...
                for row in rows:
                    ...
                    for date_col in ['entry_time', 'funding_flip_start_time']:
                        ...
                        # Normalize several possible stored date formats:
                        # - ISO strings (with or without trailing Z)
                        # - DB string formatted as '%Y-%m-%d %H:%M:%S.%f'
                        # - Integer/float unix timestamps (seconds)
                        # - None / empty -> keep as None
    ...
    async with lock:
        ...
        if success1 and success2:
            ...
            try:
                ...
                logger.info(f"{symbol} saved to DB")
