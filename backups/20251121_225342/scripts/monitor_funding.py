...
POSITION_CACHE_TTL = 30  # Sekunden (30s = guter Balance zwischen Speed & Rate Limits)
...
async def execute_trade(opp: Dict, lighter: LighterAdapter, x10: X10Adapter) -> bool:
    ...
    async with lock:  # 🔒 CRITICAL SECTION START
        ...
        if symbol in FAILED_COINS:
            last_fail_time = FAILED_COINS[symbol]
            if time.time() - last_fail_time < 300:  # 5 Min Blacklist
                logger.debug(f"⏳ {symbol} still blacklisted (failed {time.time()-last_fail_time:.0f}s ago)")
                return False
            else:
        ...
        # ===== 🆕 PRE-CALCULATE MIN REQUIREMENTS =====
        ...
        if absolute_min_required > config.MAX_TRADE_SIZE_USD:
            logger.debug(
                f"⏭️ Skip {symbol}: Min ${absolute_min_required:.2f} > "
                f"Max ${config.MAX_TRADE_SIZE_USD:.2f}"
            )
            return False
        ...
        # Determine Exchanges
        # ...ex1, bal1, ex2, bal2 sind bereits gesetzt...

        # ===== 🆕 BEIDE EXCHANGES MÜSSEN GENUG HABEN =====
        min_required_per_exchange = final_usd * 1.10  # 10% buffer

        if bal1 < min_required_per_exchange:
            logger.debug(
                f"⚠️ {symbol}: {ex1.name} balance insufficient "
                f"(${bal1:.2f} < ${min_required_per_exchange:.2f} required)"
            )
            return False

        if bal2 < min_required_per_exchange:
            logger.debug(
                f"⚠️ {symbol}: {ex2.name} balance insufficient "
                f"(${bal2:.2f} < ${min_required_per_exchange:.2f} required)"
            )
            return False

        logger.debug(
            f"✅ {symbol} Balance Check: "
            f"{ex1.name}=${bal1:.2f}, {ex2.name}=${bal2:.2f} "
            f"(Required: ${min_required_per_exchange:.2f} each)"
        )
        ...
        try:
            ...
            POSITION_CACHE['last_update'] = 0.0  # Invalidate cache
            logger.debug(f"🔄 Position cache invalidated after {symbol} execution")
...
async def get_cached_positions(lighter, x10, force_refresh: bool = False) -> Tuple[List[dict], List[dict]]:
    """
    ✅ PRODUCTION-GRADE: Cached Position Fetching mit Rate Limit Protection
    
    Args:
        lighter: Lighter Adapter
        x10: X10 Adapter
        force_refresh: Force cache refresh (für startup/reconciliation)
    ...
    async with POSITION_CACHE_LOCK:
        now = time.time()
        
        # Check if cache is fresh
        if not force_refresh and (now - POSITION_CACHE['last_update']) < POSITION_CACHE_TTL:
            logger.debug(
                f"📦 Using cached positions "
                f"(age: {now - POSITION_CACHE['last_update']:.1f}s)"
            )
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"⚠️ X10 position fetch error: {e}. Using cached data.")
                x10_positions = POSITION_CACHE.get('x10', [])
            
            try:
                lighter_positions = await lighter.fetch_open_positions()
            except Exception as e:
                logger.warning(f"⚠️ Lighter position fetch error: {e}. Using cached data.")
            ...
            logger.debug(
                f"✅ Position cache updated: "
                f"X10={len(POSITION_CACHE['x10'])}, "
                f"Lighter={len(POSITION_CACHE['lighter'])}"
            )
...
async def manage_open_trades(lighter, x10):
    """
    ✅ PRODUCTION-GRADE: Managed offene Trades mit Position Reconciliation
    
    Flow:
    1. Hole DB Trades
    2. Hole echte Exchange Positions
    3. Reconcile (finde manuell geschlossene Trades)
    4. Check Exit Signals (Stop Loss, Take Profit, Funding Flip)
    5. Execute Closes
    """
    ...
    try:
        x10_positions, lighter_positions = await get_cached_positions(lighter, x10)
