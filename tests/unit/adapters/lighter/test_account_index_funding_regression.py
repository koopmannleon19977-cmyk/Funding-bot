...
class TestAccountIndexZeroFundingValidation:
    ...
    def test_account_index_zero_passes_validation(self):
        ...
        assert validation_result is False, "account_index=0 should PASS validation (is None check returns False)"

        # This means funding collection proceeds
        should_proceed_to_funding_fetch = not validation_result
        assert should_proceed_to_funding_fetch is True, "Funding collection should proceed when account_index=0"
    ...
    def test_account_index_none_fails_validation(self):
        ...
        assert validation_result is True, "account_index=None should FAIL validation (is None check returns True)"

        # This means funding collection is skipped
        should_skip_funding_fetch = validation_result
        assert should_skip_funding_fetch is True, "Funding collection should be skipped when account_index=None"
    ...
    def test_account_index_positive_passes_validation(self):
        ...
        assert validation_result is False, "account_index=254 should PASS validation (is None check returns False)"

        # This means funding collection proceeds
        should_proceed_to_funding_fetch = not validation_result
        assert should_proceed_to_funding_fetch is True, "Funding collection should proceed when account_index=254"
    ...
    def test_negative_account_index_passes_validation_but_api_will_reject(self):
        ...
        assert validation_result is False, "account_index=-1 passes None check (API will reject invalid index)"
...
class TestRegressionBuggyValidationLogic:
    ...
    def test_old_buggy_logic_blocks_account_zero(self):
        ...
        assert old_result is True, "OLD BUG: account_index=0 was incorrectly BLOCKED by <= 0 check"

        # NEW: account_index=0 is ALLOWED (False = don't skip)
        assert new_result is False, "NEW FIX: account_index=0 is correctly ALLOWED by is None check"
    ...
    def test_old_buggy_logic_would_crash_on_none(self):
        ...
        assert new_result is True, "NEW FIX: account_index=None is handled gracefully (correctly disabled)"

        print("\n✅ PROVEN REGRESSION:")
        print("  Old code (<= 0): CRASHES on account_index=None (TypeError)")
    ...
    def test_distinguishing_disabled_from_valid(self):
        ...
        assert old_none_result == old_zero_result, "OLD BUG: None and 0 were indistinguishable (both blocked)"
...
class TestFundingCollectionDecisionMatrix:
    ...
    def test_funding_collection_decision_matrix(self, account_index, account_api_present, expected_decision, reason):
        """
...
class TestRealWorldScenarios:
    ...
    def test_scenario_default_lighter_user_with_account_zero(self):
        ...
        assert validation_blocks is False, "Default user (account_index=0) should be ALLOWED to collect funding"
    ...
    def test_scenario_explicitly_disabled_account(self):
        ...
        print(f"  Validation: {'ALLOWED' if not validation_blocks else 'BLOCKED'}")
        print(
            f"  Impact: {'✅ Can collect funding' if not validation_blocks else '❌ Cannot collect funding (intentional)'}"
        )
