...
from src.pnl_utils import compute_hedge_pnl, compute_realized_pnl, sum_closed_pnl_from_csv
...
class TestPnLRecording:
    ...
    async def test_pnl_recording_basic(self, tmp_path):
        ...
        cursor.execute("SELECT symbol, pnl, funding_collected, closed_at FROM trades WHERE symbol = ?", (test_symbol,))
        row = cursor.fetchone()
