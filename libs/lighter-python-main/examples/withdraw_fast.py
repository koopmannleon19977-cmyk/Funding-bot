...
async def main():
    ...
    try:
        ...
        fee_info = await info_api.transfer_fee_info(
            account_index=client.account_index, to_account_index=to_account, auth=auth_token
        )
        ...
        params = api_client.param_serialize(
            ...
            post_params=[("tx_info", tx_info_str), ("to_address", WITHDRAW_ADDRESS)],
            header_params={"Authorization": auth_token, "Content-Type": "application/x-www-form-urlencoded"},
        )
        response = await api_client.call_api(*params)
        await response.read()
        data = response.data
