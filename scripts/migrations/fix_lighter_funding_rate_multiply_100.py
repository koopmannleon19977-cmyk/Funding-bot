#!/usr/bin/env python3
"""
Fix Lighter funding rates that were incorrectly divided by 100.

BUG: Lighter API returns funding rates in DECIMAL format (e.g., -0.000386),
     but the code was dividing by 100, making rates 100× too small.

FIX: Multiply all Lighter funding_rate_hourly and funding_apy by 100.

This migration:
1. Creates backup of funding_history table
2. Multiplies Lighter rates by 100
3. Validates the fix
4. Provides rollback option

Usage:
    python scripts/migrations/fix_lighter_funding_rate_multiply_100.py
"""
...
def analyze_before_migration(db_path: Path) -> dict:
    ...
    print(f"  Total Lighter Records: {results['total_records']}")
...
def migrate_lighter_rates(db_path: Path, dry_run: bool = False) -> bool:
    """Multiply all Lighter funding rates by 100 to fix the bug.
...
def verify_after_migration(db_path: Path) -> dict:
    ...
    cursor.execute("""
        ...
        FROM funding_history
        WHERE exchange = 'LIGHTER' AND symbol = 'BERA'
        GROUP BY symbol
    """)
...
def main():
    ...
    print("=" * 100)
    print("LIGHTER FUNDING RATE BUG FIX MIGRATION")
    print("=" * 100)
    print("\nThis migration fixes Lighter funding rates that were incorrectly divided by 100.")
    print("All Lighter rates will be multiplied by 100 to correct the bug.\n")
    ...
    if before_stats["total_records"] == 0:
        print("\n[INFO] No Lighter records found - nothing to migrate.")
        print("  This is normal if the bot hasn't fetched any Lighter data yet.")
        return 0
    ...
    print(f"\nStep 3: Ready to migrate {before_stats['total_records']} Lighter records.")
    ...
    if not migrate_lighter_rates(db_path, dry_run=False):
    ...
    print("2. The bot will now fetch NEW Lighter data correctly (no /100 division)")
