...
async def main():
    ...
    await asyncio.gather(x10.load_market_cache(force=True), lit.load_market_cache(force=True))
    ...
    targets = ["ZORA-USD", "SHIB-USD", "SPX-USD", "CRV-USD", "SEI-USD", "ADA-USD", "WLD-USD", "EIGEN-USD", "TRUMP-USD"]
    ...
    for sym in found[:8]:
        try:
            ...
            print(
                f"{sym}: X10={float(x10_rate) * 100:.4f}%/h, Lighter={float(lit_rate) * 100:.4f}%/h | Net APY={apy:.1f}% | {direction}"
            )
