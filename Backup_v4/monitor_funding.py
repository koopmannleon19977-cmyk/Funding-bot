    ...
    for trade in open_trades.copy():
        if trade.get('is_farm_trade'):
            ...
            if hold_seconds > config.FARM_HOLD_SECONDS:
                ...
                await adapter1.close_live_position(trade['symbol'], trade['leg1_side'], trade['notional_usd'])
                update_trade_status(trade['symbol'], 'CLOSED_FARM')
                open_trades.remove(trade)

    for trade in open_trades:
        # Robust datetime parsing
        entry_time_str = trade['entry_time']  # Der String aus DB
        try:
            entry_time = datetime.fromisoformat(entry_time_str)  # Für ISO-Format: '2025-11-18T21:51:00'
            # Oder, wenn Format 'YYYY-MM-DD HH:MM:SS' (ohne T): entry_time = datetime.strptime(entry_time_str, '%Y-%m-%d %H:%M:%S')
        except ValueError as ve:
            logger.error(f"Ungültiges entry_time Format für {trade['symbol']}: {entry_time_str} | Fehler: {ve}")
            continue  # Überspringe diesen Trade, um Crash zu vermeiden
