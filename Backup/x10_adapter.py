...
class X10Adapter(BaseAdapter):
    def __init__(self):
        ...
        try:
            if config.X10_VAULT_ID:
                ...
                logger.info("✅ X10 Account erfolgreich initialisiert.")
            else:
                logger.warning("⚠️ X10 Config fehlt (Vault ID leer). Account nicht initialisiert.")
        except Exception as e:
            logger.error(f"❌ X10 Account Init Error: {e}")

    async def get_order_fee(self, order_id: str) -> float:
        """
        Gibt die Fee zurück basierend auf der offiziellen 'Flat Fee Structure'.
        Quelle: Extended Docs (Maker 0.000%, Taker 0.025%).
        """
        # Da wir Post-Only (Maker) nutzen, zahlen wir 0.00% Gebühren.
        # Wir sparen uns den API-Call, der aktuell Fehler wirft.
        return 0.0000

    async def start_websocket(self):
        """
        X10 Extended WebSocket (Path-Based)
        URL: wss://api.starknet.extended.exchange/stream.extended.exchange/v1/publicTrades
        Logik: Connect = Subscribe. Wir erhalten Trades für ALLE Märkte.
        """
                ...
                async with session.ws_connect(url) as ws:
                    ...
                    async for msg in ws:
                        if msg.type == aiohttp.WSMsgType.TEXT:
                            ...
                            logger.error(f"X10 WS Stream Error: {msg}")
                            break
            except Exception as e:
    ...
    async def _get_auth_client(self) -> PerpetualTradingClient:
        if not self._auth_client:
            if not self.stark_account: raise RuntimeError("X10 Stark account missing")
    ...
    async def load_market_cache(self, force: bool = False):
        if self.market_info and not force: return
        ...
        try:
            ...
            if resp and resp.data:
                for m in resp.data:
                    name = getattr(m, "name", "")
                    if name.endswith("-USD"): self.market_info[name] = m
    ...
    def fetch_funding_rate(self, symbol: str):
        """
        X10 Extended liefert STÜNDLICHE (1h) Funding Rates.
        Quelle: https://api.docs.extended.exchange/#markets-info
        """
        m = self.market_info.get(symbol)
        # NEU: Funding ist stündlich (laut Docs), also 1:1 übernehmen
    ...
    def fetch_24h_vol(self, symbol: str) -> float:
        try:
            m = self.market_info.get(symbol)
            if m: return abs(float(getattr(m.market_stats, "price_change_24h_pct", 0)))
        except: pass
        return 0.0

    def get_24h_change_pct(self, symbol: str = "BTC-USD") -> float:
        """
        Gibt die 24h-Preisänderung in Prozent zurück (z.B. 5.2 für +5.2%).
        Nutzt standardmäßig BTC als Markt-Indikator.
        """
        try:
            # Wir schauen in die Market-Info, die wir eh geladen haben
            m = self.market_info.get(symbol)
            # Das Attribut heißt oft price_change_24h_pct (X10 Standard)
            if m and hasattr(m, "market_stats"):
                val = getattr(m.market_stats, "price_change_24h_pct", 0)
                return float(str(val)) * 100.0 # Oft als 0.05 geliefert -> 5.0%
        except:
            pass
        return 0.0
    ...
    def min_notional_usd(self, symbol: str) -> float:
        """
        Gibt das Minimum in USD zurück. 
        FIX: Erzwingt mindestens $15, egal was die API sagt.
        """
        HARD_MIN_USD = 15.0 # Sicherheitsnetz
        
        m = self.market_info.get(symbol)
        if not m: return HARD_MIN_USD

        try:
            # Versuch 1: API Config
            ...
            # Wir nehmen das GRÖSSERE von beiden: API-Wert oder Hard-Limit
            return max(api_min, HARD_MIN_USD)
        except: 
            return HARD_MIN_USD

    async def fetch_open_positions(self) -> list:
        if not self.stark_account: return []
        try:
            ...
            logger.info(f"X10: {len(positions)} echte offene Positionen")
            return positions
        except Exception as e:
            logger.error(f"X10 Positions Error: {e}")
            return []

    async def open_live_position(self, symbol: str, side: str, notional_usd: float, reduce_only: bool = False, post_only: bool = True) -> Tuple[bool, Optional[str]]:
        if not config.LIVE_TRADING: return True, None
        client = await self._get_auth_client()
        market = self.market_info.get(symbol)
        if not market: return False, None

        price = Decimal(str(self.fetch_mark_price(symbol) or 0))
        if price <= 0: return False, None
        ...
        if hasattr(cfg, "round_price") and callable(cfg.round_price):
            try: limit_price = cfg.round_price(raw_price)
            except: limit_price = raw_price.quantize(Decimal("0.01"), rounding=ROUND_UP if side == "BUY" else ROUND_DOWN)
        else:
            tick_size = Decimal(getattr(cfg, "min_price_change", "0.01"))
            if side == "BUY": limit_price = ((raw_price + tick_size - Decimal('1e-12')) // tick_size) * tick_size
            else: limit_price = (raw_price // tick_size) * tick_size
        ...
        if step > 0:
            qty = (qty // step) * step
            if qty < min_size: qty = ((qty // step) + 1) * step

        order_side = OrderSide.BUY if side == "BUY" else OrderSide.SELL
        tif = TimeInForce.GTT
        if post_only:
            if hasattr(TimeInForce, "POST_ONLY"): tif = TimeInForce.POST_ONLY
            elif hasattr(TimeInForce, "PostOnly"): tif = TimeInForce.PostOnly
            else: post_only = False 
        ...
        try:
            ...
            if resp.error:
                ...
                if reduce_only and ("1137" in err_msg or "1138" in err_msg or "Position is missing" in err_msg or "Position is same side" in err_msg):
                    logger.warning(f"X10: Position für {symbol} war schon zu (Error 1137/1138). Betrachte als Erfolg.")
                    return True, None
                
                logger.error(f"X10 Order Fail: {resp.error}")
                if post_only and "post only" in err_msg.lower():
                    logger.info("Retry without PostOnly...")
                    return await self.open_live_position(symbol, side, notional_usd, reduce_only, post_only=False)
                return False, None
                
            logger.info(f"X10 Order OK: {resp.data.id}")
            return True, str(resp.data.id)
        except Exception as e:
            err_str = str(e)
            if reduce_only and ("1137" in err_str or "1138" in err_str or "Position is missing" in err_str or "Position is same side" in err_str):
                return True, None
            logger.error(f"X10 Order Exception: {e}")
            return False, None

    async def close_live_position(self, symbol: str, original_side: str, notional_usd: float) -> Tuple[bool, Optional[str]]:
        close_side = "SELL" if original_side == "BUY" else "BUY"
        return await self.open_live_position(symbol, close_side, notional_usd, reduce_only=True, post_only=False)
    ...
    async def get_real_available_balance(self) -> float:
        try:
            ...
            if hasattr(resp, 'data'):
                 ...
                 if available: return float(str(available))
            return 0.0
        except Exception as e:
    ...
    async def aclose(self):
        if self._auth_client: await self._auth_client.close()